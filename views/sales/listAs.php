							...
							<table id="estimate_list" class="table  table-bordered table-striped">
								<thead class="thin-border-bottom">
									<tr>
										...
										<?}?>
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 접수번호</th>
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 접수일자</th>
										<th class="col-xs-3"><i class="ace-icon fa fa-caret-right blue"></i> 품목명</th>
										<th class="col-xs-3"><i class="ace-icon fa fa-caret-right blue"></i> 거래처</th>
										...
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 처리담당자</th>
					...
					<div class="clearfix form-actions center" style="margin-top:0px">
						<div class="col-md-12">
							...
							<div class="col-xs-6 right" style="text-align:right">
								<button class="btn btn-info" type="button" onclick="location.href = 'index.php?controller=sales&action=inputPageAs'">
...
function getAs(page){
	...
	$.getJSON("ajax/sales.php",{"page":page, "mode":"getAs", "rpp" : rpp, "adjacents" : adjacents, "where" : where},
		function(json){
			if(json != null) {
				...
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "</td>";
					<?}?>
					tag += "<td><a href='index.php?controller=sales&action=modifyPageAs&uid=" + json[i].uid + "'>" + json[i].accept_cd + "</a></td>";
...
function search(){
	...
	$("#where").val(" where account_nm like '%" + txt + "%' or item_nm like '%" + txt + "%' or item_nm like '%" + txt + "%' ");
	getAs(1);
}
</script>
