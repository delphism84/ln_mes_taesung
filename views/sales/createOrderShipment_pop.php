			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form name="frm" id="frm" method="post" action="index.php" enctype="multipart/form-data">
						...
						<input type="hidden" name="action" id="action" value="inputPageOrderShipment" />
						<input type="hidden" name="dialogID" id="dialogID" value="<?=$_POST["dialogID"]?>" />
						...
						<table id="simple-table" class="table  table-bordered table-hover">
							<tr>
								<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 출하일자</th>
								<td class="col-xs-5" colspan="3">
									<!-- <input type="text" name="order_cd" id="order_cd" readonly /> -->
									<div>
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input class=" date-picker" name="shipment_dt" id="shipment_dt" type="text" data-date-format="yyyy/mm/dd" value='<?=date("Y/m/d")?>'/>
							...
							<tr>
								<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 주문서코드</th>
								<td class="col-xs-5">
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input type="hidden" name="order_uid" id="order_uid" readonly />
												<input type="text" name="order_cd" id="order_cd" onclick="centerOpenWindow('views/popup/orderListPut.php', '주문서서리스트', 700, 500)" readonly />
								...
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											...
											<input type="text" name="warehouse_nm" id="warehouse_nm" onclick="centerOpenWindow('views/popup/warehouseList.php', '창고리스트', 600, 500)" readonly/>
							...
							<tr>
								...
								<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right blue"></i> 납품기한</th>
								<td class="col-xs-5">
									<div>
										<span class="input-icon input-icon-right">
											<div class="input-group">
							...
							<tr>
								...
								<td class="col-xs-5"><input type="text" name="mobile" id="mobile" /></td>
								<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 출하예정일</th>
								<td class="col-xs-5">
									<div>
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input class=" date-picker" name="consignment_dt" id="consignment_dt" type="text" data-date-format="yyyy-mm-dd" value="<?=date('Y-m-d')?>" />
												<span class="input-group-addon">
													<i class="fa fa-calendar bigger-110"></i>
												</span>
											</div>
							...
							<tr>
								...
								<td class="col-xs-11" colspan="3" >
									<div class="input-group col-xs-2">
										<input type="text" class="search-query form-control" placeholder="우편번호" name="zipcode" id="zipcode" style="width:100px" readonly />
									...
									<div style="margin-top:5px">
										<input type="text" class="search-query form-control" placeholder="주소" name="address" id="address" style="width:50%" />
									</div>
								</td>
							</tr>
							<tr>
						...
						<a class="btn btn-xs btn-success" onclick="centerOpenWindow('views/popup/shipmentItemList.php', '품목리스트', 1000, 500)">품목추가</a>
...
function calculation(flag) {
	//alert( $("#cnt_" + flag).val() );
	//alert( $("#remain_cnt_"+flag).val() );
	var tax_type = $("#tax_type option:selected").val();
	var cnt = $("#cnt_" + flag).val();

	var remain_cnt = ($("#remain_cnt_"+flag).val());

	if(Number(removeComma(cnt)) > Number(removeComma(remain_cnt))){
		$("#cnt_" + flag).val( remain_cnt );
	}
...
function getOrderItem(){
	...
	var flag = $("#flag").val();
	var tag2 = "";
	$.getJSON("ajax/order.php",{"mode":"getOrderItem2", "uid" : uid},
		function(json){
			
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					
					tag2 += "<tr class='item" + flag + "'>";
					tag2 += "<td class='center'><i class='delBtn fa fa-minus-square fa-2x' aria-hidden='true' onclick='delTr(this)'></i></td>";
					tag2 += "<td><input type='text' class='form-control id-btn-dialog item_cd ' name='item_cd[]' id='item_cd_" + flag + "' onclick='viewModal(), itemFlag(" + flag + ")'  value='" + json[i].item_cd + "' readonly /></td>";
					tag2 += "<td><input type='text' class='form-control' name='item_nm[]' id='item_nm_" + flag + "' value='" + json[i].item_nm + "' readonly /></td>";
					tag2 += "<td><input type='text' class='form-control' name='standard1[]' id='standard1_" + flag + "' value='" + json[i].standard1 + "' onkeyup='calculation(" + flag + ")' readonly/></td>";
					...
					tag2 += "<td><input type='text' class='form-control' name='unit[]' id='unit_" + flag + "' value='" + json[i].unit + "' onkeyup='calculation(" + flag + ")' readonly/></td>";
					tag2 += "<td><input type='text' class='form-control cnt text-right' name='cnt[]' id='cnt_" + flag + "' value='" + json[i].cnt + "' onkeyup='calculation(" + flag + ");input_comma(this);' onclick='this.select();' onkeydown='calculationTotal(" + flag + ");' /></td>";
					tag2 += "<td><input type='text' class='form-control text-right' name='remain_cnt[]' id='remain_cnt_" + flag + "'  value='"+ json[i].remain_cnt +"' readonly/></td>";
					tag2 += "<td><input type='text' class='form-control text-right' name='box_cnt[]' id='box_cnt_" + flag + "'  /></td>";
					//tag += "<td><input type='text' class='form-control' name='remark[]' id='remark_" + flag + "' /></td>";
					tag2 += "<td><input type='text' class='form-control lot_no' name='lot_no_cd[]' id='lot_no_cd_" + flag + "' onclick=\"lotnocdFlag2(" + flag + ");lot_no_reg(" + flag + ",'"+ json[i].item_cd +"','"+ json[i].standard1 +"','"+ json[i].remain_cnt +"')\" value='' placeholder='[click]' readonly/><input type='hidden' name='lot_no_nm[]'  id='lot_no_nm_" + flag + "'/></td>";
					tag2 +="<td><input type='text' name='warehouse_nm[]'  id='warehouse_nm_" + flag + "' onclick=\"lotnocdFlag2(" + flag + ");lot_no_reg(" + flag + ",'"+ json[i].item_cd +"','"+ json[i].standard1 +"','"+ json[i].remain_cnt +"');\" value='' placeholder='[click]' readonly/><input type='hidden' name='warehouse_cd[]'  id='warehouse_cd_" + flag + "' value='' readonly/></td>";
					tag2 += "</tr>";
...
function lot_no_reg(cidx, item_cd , standard1 , remain_cnt){
	...
	var url = "index.php?controller=base&action=listLotNoPop3&idx="+cidx+"&item_cd=" + item_cd + "&standard1="+ standard1 +" &out='out'&pop=Y&dialogID=id-btn-dialog1";
	$("#lot_no_reg_frame").attr("src", url);
}

function lotnocdFlag2(flag) {
...
function formSubmit(){
	...
	if(!check_str($("#delivery_dt").val(),"납품기한")) return false;
	...
	*/
	...
	if(!frm_submit($('input[name="warehouse_nm[]"]'),"출하창고")) return false;
