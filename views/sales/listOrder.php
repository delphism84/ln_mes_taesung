					...
					<div class="widget-header">
						<div class="col-xs-8" style="float:left">
							<div class="form-inline">
								<select id="per" class="form-control" onchange="getOrder(1)" style="float:left; ">
								...
								<div style="float:left">
									...
									<span class="input-icon input-icon-right">
										<div class="input-group">
											<button type="button" class="btn btn-purple btn-sm form-control" onclick="getOrder(1)" >
							...
							<div class="col-xs-8"  style="float:right">
								<div class="input-group">						
									...
									<span class="input-group-btn">
										<button type="button" class="btn btn-purple btn-sm" onclick="getOrder()">
											<span class="ace-icon fa fa-search icon-on-right bigger-110"></span>
											Search
										</button>
									</span>
							...
							<table id="order_list" class="table  table-bordered table-striped">
								<thead class="thin-border-bottom">
									<tr>
										...
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 주문일</th>
					...
					<div class="clearfix form-actions" style="margin-top:0px">
						<div class="col-md-12">
							...
							<div class="col-xs-6 right" style="text-align:right">
								<button class="btn btn-info" type="button" onclick="order_reg('0','0')">
        ...
        <div class="modal-content" >
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">수주서 등록</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="order_reg_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
    ...
    <div class="modal-dialog modal-lg" style="width:80%">
        <div class="modal-content" >
            <div class="modal-header" id="modal-header2">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">수주서 수정</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="order_modify_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
function getOrder(page){
	...
	$.getJSON("ajax/estimate.php",{"page":page, "mode":"getOrder", "rpp" : rpp, "adjacents" : adjacents, "where": where, "search_txt": txt, "start_dt": start_dt, "end_dt": end_dt},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					if(json[i].state != "4") tag += "<td><a href='javascript:void(0);'  onclick=\"order_modiy('" + json[i].uid + "','" + json[i].order_dt + "','" + json[i].order_cha + "','" + json[i].state + "');\">" + json[i].order_cd + "</td>";
					...
					tag += "<td class='align-right'>" + json[i].priceTotal + "</td>";

					if(json[i].state == "0") var state = "대기";
					else if(json[i].state == "1") var state = "진행중";
					else if(json[i].state == "2") var state = "출하완료";
					else if(json[i].state == "3") var state = "수주취소";
					else if(json[i].state == "4") var state = "종료";
					else var state = "진행중";
				...
				tag += "<tr>";
				tag += "<td colspan='11' class='center' style='height:20px'> 등록된 수주(주문) 리스트가 없습니다. </td>";
				tag += "</tr>";
			}
			...
			$("#order_list tbody").html(tag);
				...
				where = " where 1=1";
			if(txt != "") {
				where += " and (order_dt like '%"+txt+"%' or account_nm like '%"+txt+"%' or manager like '%"+txt+"%')";
			}
...
function endSelect(uid){
	if(confirm("선택한 수주 전표를 종결처리 하겠습니까?\n종결된 수주전표는 수정이나 삭제가 불가능합니다.")) {
		var dataString = "mode=endSelectOrder&table=erp_order&uid=" + uid;
		$.ajax({
			type : "post",
...
function endCancelSelect(uid){
	if(confirm("선택한 수주 전표를 종결 취소처리 하겠습니까?\n취소된 수주전표는 리스트에서 확인 가능 능합니다.")) {
		var dataString = "mode=endCancelSelectOrder&table=erp_order&uid=" + uid;
		$.ajax({
			type : "post",
			url : "ajax/estimate.php",
			data : dataString,
...
function est_print(est_cd, ctype)
{
	...
	if( ctype == 'SA' ) {
		...
		url = "../attach/doc_form/esti_order_print_pop.php" + param;
	}
...
<!--
	...
	var url = "index.php?controller=sales&action=registPageOrderPop&idx="+cidx+"&pop=Y&aci_cd="+acd+"&dialogID=id-btn-dialog1";
	//var url = "./views/accounting/card/cardcompany_remark_list_pop.php?&sid="+cidx+"&pop=Y&ddd="+cidx;
	$("#order_reg_frame").attr("src", url);
	}

	function order_modiy(sid, dt, ca, st)
	{
	...
	var url = "index.php?controller=sales&action=modifyPageOrderPop&uid="+sid+"&pop=Y&purchase_dt="+dt+"&purchase_cha="+ca+"&state="+st+"&dialogID=id-btn-dialog2";
	//var url = "./views/accounting/card/cardcompany_remark_list_pop.php?&sid="+cidx+"&pop=Y&ddd="+cidx;
	$("#order_modify_frame").attr("src", url);
	}

	function close_popup()
	{	
		$.modal.close();
		$("#order_reg_frame").attr("src", "about:blank");
	}
