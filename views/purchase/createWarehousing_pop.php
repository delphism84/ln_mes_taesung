			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form name="frm" id="frm" method="post" action="index.php" enctype="multipart/form-data">
						...
						<input type="hidden" name="action" id="action" value="inputWarehousingItem" />
						...
						<table id="simple-table" class="table  table-bordered table-hover">
							<tr>
									...
									<div>
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input class=" date-picker" name="warehousing_dt" id="warehousing_dt" type="text" value=<?=date('Y/m/d')?> data-date-format="yyyy/mm/dd" />
								...
								<td class="col-xs-5">
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input type="hidden" name="p_order_uid" id="p_order_uid" readonly />
												<input type="text" name="p_order_cd" id="p_order_cd" onclick="centerOpenWindow('views/popup/purchaseOrderItemList.php', '발주서 Item 리스트',900, 500)" readonly />
							...
							<tr>
								...
								<td class="col-xs-5">
									<div>
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input class=" date-picker" name="deadline_dt" id="deadline_dt" type="text" data-date-format="yyyy-mm-dd" value='<?=date('Y-m-d')?>'/>
												<span class="input-group-addon">
													<i class="fa fa-calendar bigger-110"></i>
												</span>
											</div>
								...
								<td class="col-xs-5">
									<select name="currency" id="currency">
										...
										<option value="3">엔화</option>
										<option value="4">위안</option>
										<option value="5">유로</option>
										<option value="6">추가</option>
									</select>
								</td>
							</tr>
							...
							<tr>
								...
								<td class="col-xs-10">
									...
									<label class="pl-1">
										<input name="switch-field-1" class="ace ace-switch ace-switch-6"  name="purchaseOrderAccount" id="purchaseOrderAccount" value='Y' type="checkbox" />
										<span class="lbl">&nbsp;회계 반영</span>
									</label>
									<label class="pl-2">
										<input name="switch-field-1" class="ace ace-switch ace-switch-6"  name="purchaseOrderState" id="purchaseOrderState" value='Y' type="checkbox" />
										<span class="lbl">&nbsp;발주서 종결</span>
									</label>
								</td>
							</tr>
						</table>
						...
						<span style="color:red"> * 품목의 LOT_NO는 11자리 이내 & - 대신에 _ 를 사용해 주세요</span>
...
function inspectionPop(flag,id,fid, cd,nm,st,unit,cnt )
{
 ...
 $("#inspectionFlag").val(flag);
	...
	window.open(encodeURI(url),"popup", "width=600, height=700, left=0, top=0, toolbar=no, location=no, directories=no, status=no, menubar=no, resizable=no, scrollbars=no, copyhistory=no");
}
		...
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "<td><input type='text' class='form-control supply_price text-right' name='supply_price[]' id='supply_price_" + currentFlag + "' onkeyup='calculation(" + currentFlag + ");input_comma(this);' onclick='this.select();' onblur='calculationTotal(" + currentFlag + ");' value='" + json[i].supply_price + "'  /></td>";
					...
					tag += "<td><input type='text' class='form-control lot_no' name='lot_no_cd[]' id='lot_no_cd_" + currentFlag + "' onclick='lotnocdFlag(" + currentFlag + ");lot_no_reg(" + currentFlag + ")' readonly /><input type='hidden' name='lot_no_nm[]'  id='lot_no_nm_" + currentFlag + "' placeholder='[click]'/></td>";
			...
			$("#warehousing_item tbody").html(tag);
			calculationTotal();
		}
	);
}
