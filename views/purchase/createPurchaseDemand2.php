				...
				<div class="col-xs-12">
					...
					<form name="frm" id="frm" method="post" action="index.php" enctype="multipart/form-data">
									...
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												...
												<input type="text" name="order_cd" id="order_cd" onclick="centerOpenWindow('views/popup/orderList.php', '주문서리스트', 600, 500)" readonly />
												<span class="input-group-addon btn-purple" onclick="centerOpenWindow('views/popup/orderList.php', '주문서리스트', 600, 500)">
							...
							<tr>
								...
								<td class="col-xs-5">
									<div>
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input class=" date-picker" name="purchase_expect_dt" id="purchase_expect_dt" type="text" data-date-format="yyyy-mm-dd" />
												<span class="input-group-addon">
													<i class="fa fa-calendar bigger-110"></i>
												</span>
											</div>
						...
						<table id="simple-table" class="table  table-bordered table-hover">
								...
								?>
								<tr>
									<td><span class="label label-sm label-pink arrowed-right" style="cursor:pointer" onclick="getBom('<?=$item->item_cd?>','<?=$item->item_nm?>','<?=$item->standard1?>',<?=$item->cnt?>)">소요량 계산</span> <strong><?=$item->item_nm?></strong>
									</td>
									<td><?=$item->cnt?></td>
									<td><?=$item->cnt?></td>
									<td><?=$item->cnt?></td>
						...
						<table id="product" class="table  table-bordered table-hover" style="margin-top:10px">
							<thead>
								<tr>
									...
									<th class="center col-xs-1" style="background-color:#f1f1f1">규격1</th>
									<th class="center col-xs-1" style="background-color:#f1f1f1">규격2</th>
...
function formSubmit(){
	...
	if(!check_str($("#purchase_cd").val(),"구매요청코드")) return false;
