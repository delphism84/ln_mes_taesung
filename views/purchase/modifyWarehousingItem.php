...
function inspectionModifyPop(flag,cd)
{
 $("#inspectionFlag").val(flag);

 window.open("views/purchase/receivingInspection_modify.php?flag="+flag+"&cd="+cd,"popup", "width=600, height=600, left=0, top=0, toolbar=no, location=no, directories=no, status=no, menubar=no, resizable=no, scrollbars=no, copyhistory=no");
}
		...
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "<td><input type='text' class='form-control item_cd ' name='item_cd[]' id='item_cd_" + flag + "' onclick='viewModal();itemFlag(" + flag + ")' value='" + json[i].item_cd + "' placeholder='품목선택을 하시려면 클릭하세요' readonly /></td>";
					...
					tag += "<td><input type='text' class='form-control lot_no id-btn-dialog' name='lot_no_nm[]' id='lot_no_nm_" + flag + "' onclick='lotnocdFlag(" + flag + ")' value='" + json[i].lot_no_nm + "' readonly /><input type='hidden' name='lot_no_cd[]'  id='lot_no_cd_" + flag + "' value='" + json[i].lot_no_cd + "' /></td>";
					tag += "<td><input type='text' class='form-control' name='inspection_cd[]' id='inspection_cd_" + flag + "' onclick=\"inspectionModifyPop(" + flag + ",'" + json[i].inspection_cd + "')\" value='" + json[i].inspection_cd + "' readonly /></td>";
					tag += "</tr>";
