			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form name="frm" id="frm" method="post" action="index.php" enctype="multipart/form-data">
							...
							<tr>
								...
								<td class="col-xs-5">
									<div>
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input class=" date-picker" name="purchase_expect_dt" id="purchase_expect_dt" type="text" data-date-format="yyyy-mm-dd" value='<?=date('Y-m-d')?>' readonly/>
												<span class="input-group-addon">
													<i class="fa fa-calendar bigger-110"></i>
												</span>
											</div>
						...
						<a class="btn btn-xs btn-success" onclick="centerOpenWindow('views/popup/workPlanItemList.php', '품목리스트', 800, 600)">품목선택</a>
						<table id="workplan_item_bom" class="table  table-bordered table-hover" style="margin-top:10px">
							<thead>
								<tr>
									...
									<th class="center col-xs-1" style="background-color:#f1f1f1;width:5%" nowrap>재고수량</th>
									<th class="center col-xs-1" style="background-color:#f1f1f1;width:5%" nowrap>부족수량</th>
...
function getWorkPlanItemBom(){
	...
	$.getJSON("ajax/purchase.php",{"mode":"getWorkPlanItemBom", "wid" : uid},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
						//alert(json[i].item_nm);	//2018.10.16 내역2개 이상이어도 1개만 불러와서 확인.
						tag = "<tr class='item" + currentFlag + "'>";
					...
					$("#workplan_item_bom").append(tag);	//bom 내역 한개만 불러와져서 수정 2018.10.16
					currentFlag = Number(currentFlag) + 1;
					$("#flag").val(currentFlag);
				}
			}else{
				alert('등록된 BOM이 없거나, 생산 계획 정보의 품목의 규격을 확인하세요.');
				tag += "<tr>";
				tag += "<td colspan='14' class='center'>등록된 품목정보가 없거나 규격이 일치하는 품목이 없습니다. 규격을 확인해보세요.</td>";
				tag += "</tr>";
...
function formSubmit(){
	...
	if(!frm_submit($('input[name="account_cd[]"]'),"거래처")) return false;
