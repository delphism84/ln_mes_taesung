			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form name="frm" id="frm" method="post" action="index.php" enctype="multipart/form-data">
						...
						<input type="hidden" name="action" id="action" value="updatePageMoldRepair" />
						...
						<table id="simple-table" class="table  table-bordered table-hover">
							<tr>
								...
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											<input type="hidden" name="mold_cd" id="mold_cd" value="<?=$t->mold_cd?>" readonly/>
											<input type="text" name="mold_nm" id="mold_nm" value="<?=$t->mold_nm?>" placeholder="금형코드" onclick="centerOpenWindow('views/popup/moldListPut.php', '금형코드', 500, 600)" readonly/>
								...
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											<input class=" date-picker" name="deadline_dt" id="deadline_dt" type="text" value="<?=$t->deadline_dt?>" data-date-format="yyyy/mm/dd" readonly/>
							...
							<tr>
								...
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											<input class=" date-picker" name="mold_repair_dt" id="mold_repair_dt" type="text" value="<?=$t->mold_repair_dt?>" data-date-format="yyyy/mm/dd" />
											<span class="input-group-addon">
												<i class="fa fa-calendar bigger-110"></i>
											</span>
										</div>
								...
								<td class="col-xs-5">
									<div>
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<span><strong>시작일자</strong> : </span><input class=" date-picker" name="start_dt" id="start_dt" type="text" data-date-format="yyyy-mm-dd" placeholder="시작일자" value="<?=$t->start_dt?>" style="width:100px"/>
												<span class="input-group-addon">
													<i class="fa fa-calendar bigger-110"></i>
												</span>
											</div>
										...
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<span><strong>종료일자</strong> : </span><input class=" date-picker" name="end_dt" id="end_dt" type="text" data-date-format="yyyy-mm-dd" placeholder="종료일자" value="<?=$t->end_dt?>" style="width:100px"/>
												<span class="input-group-addon">
													<i class="fa fa-calendar bigger-110"></i>
												</span>
											</div>
							...
							<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 수리유형</th>
								<td class="col-xs-5">
									<select name="repair_type" id="repair_type" >
										<option value="1" <? if($t->repair_type == 1) echo "selected"; ?>>부품파손/마모</option>
										<option value="2" <? if($t->repair_type == 2) echo "selected"; ?>>도금/열처리</option>
										<option value="3" <? if($t->repair_type == 3) echo "selected"; ?>>볼트/너트풀림</option>
										<option value="4" <? if($t->repair_type == 4) echo "selected"; ?>>표면불량</option>
										<option value="5" <? if($t->repair_type == 5) echo "selected"; ?>>강도저하</option>
										<option value="6" <? if($t->repair_type == 6) echo "selected"; ?>>탄화현상</option>
										<option value="7" <? if($t->repair_type == 7) echo "selected"; ?>>물성저하</option>
										<option value="8" <? if($t->repair_type == 8) echo "selected"; ?>>스크랩</option>
										<option value="9" <? if($t->repair_type == 9) echo "selected"; ?>>부품교체</option>
										<option value="10" <? if($t->repair_type == 10) echo "selected"; ?>>세척</option>
									</select>
								</td>
							
							<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 수리시간</th>
								<td class="col-xs-5">
									<input class="text-right" type="text" name="repair_time" id="repair_time" value="<?=$t->repair_time?>" />분
								</td>
							</tr>
							<tr>
							<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 결함구분</th>
								<td class="col-xs-5">
									<select name="defect_gb" id="defect_gb">
										<option value="1" <? if($t->defect_gb == 1) echo "selected"; ?>>치수상결함</option>
										<option value="2" <? if($t->defect_gb == 2) echo "selected"; ?>>외관상결함</option>
										<option value="3" <? if($t->defect_gb == 3) echo "selected"; ?>>내부결함</option>
										<option value="4" <? if($t->defect_gb == 4) echo "selected"; ?>>재질상결함</option>
										<option value="5" <? if($t->defect_gb == 5) echo "selected"; ?>>기타결함</option>
									</select>
								</td>
							<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 수리구분</th>
								<td class="col-xs-5">
									<select name="repair_gb" id="repair_gb">
										<option value="1" <? if($t->repair_gb == 1) echo "selected"; ?>>유지보수</option>
										<option value="2" <? if($t->repair_gb == 2) echo "selected"; ?>>정기점검</option>
										<option value="3" <? if($t->repair_gb == 3) echo "selected"; ?>>스페어파트</option>
									</select>
								</td>
							</tr>
							<tr>
							<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 결함내용</th>
								<td class="col-xs-5">
									<textarea id="defect_content" name="defect_content" class="form-control"><?=$t->defect_content?></textarea>
								</td>
							
							<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 수리내용</th>
								<td class="col-xs-5">
									<textarea id="repair_content" name="repair_content" class="form-control"><?=$t->repair_content?></textarea>
								</td>
							</tr>
							<tr>
								...
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											...
											<input type="text" name="warehouse_nm" id="warehouse_nm" value="<?=$t->warehouse_nm?>" placeholder="수리공장검색" onclick="centerOpenWindow('views/popup/warehouseList.php', '현보관처', 500, 600)" />
								...
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											...
											<input type="text" name="process_nm" id="process_nm" value="<?=$t->process_nm?>" placeholder="수리공정검색" value="<?=$t->machine_nm?>" onclick="centerOpenWindow('views/popup/processListPut.php', '공정리스트', 400, 600)" readonly/>
							...
							<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right blue"></i> 수리작업장</th>
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											<input type="hidden" name="workshop_cd" id="workshop_cd"  value="<?=$t->workshop_cd?>"/>
											<input type="text" name="workshop_nm" id="workshop_nm" value="<?=$t->workshop_nm?>" placeholder="수리작업장검색" onclick="centerOpenWindow('views/popup/warehouseListPutAny2.php?cd=workshop_cd&nm=workshop_nm', '수리작업장', 500, 600)" />
									...
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												...
												<input type="text" name="account_nm" id="account_nm" value="<?=$t->account_nm?>" placeholder="수리거래처검색" onclick="centerOpenWindow('views/popup/accountList.php', '수리거래처', 600, 500)" readonly />
							...
							<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 수리부서</th>
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											<input type="hidden" name="department_cd" id="department_cd" value="<?=$t->department_cd?>" readonly />
											<input type="text" name="department_nm" id="department_nm" value="<?=$t->department_nm?>" placeholder="수리부서검색" onclick="centerOpenWindow('views/popup/departmentListPut2.php', '수리부서', 400, 500)" readonly />
								...
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											<input type="hidden" name="manager_id" id="manager_id" value="<?=$t->manager_id?>"readonly />
											<input type="text" name="manager_nm" id="manager_nm" value="<?=$t->manager_nm?>" placeholder="수리사원검색" onclick="centerOpenWindow('views/popup/employeeListPutAny2.php?cd=manager_id&nm=manager_nm', '수리사원', 600, 500)" readonly />
							...
							<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 수리담당자</th>
									...
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												...
												<input type="text" name="emp_nm" id="emp_nm" value="<?=$t->emp_nm?>" placeholder="수리담당자검색" onclick="centerOpenWindow('views/popup/employeeListPutAny2.php', '수리담당자', 600, 500)" readonly />
								...
								<td class="col-xs-5"><input class="text-right" onkeyup='input_comma(this);' onclick='this.select();' type="text" name="repair_price" id="repair_price" value="<?=$t->repair_price?>" /></td>
							</tr>
							<tr>
							<tr>
							...
							<tr>
								...
								<td class="col-xs-5"><input type="file" name="attach" id="attach" />
								<?
									if (!empty($t->attach)) {
...
function getMoldRepairItemList(){
	var uid = $("#uid").val();
	var rpp = 15;
	var adjacents = 4;
	var tag = "";
	...
	$.getJSON("ajax/mold.php",{"mode":"getMoldRepairItemList", "rpp" : rpp, "adjacents" : adjacents, "uid" : uid},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "<td><input type='text' class='form-control item_cd' name='item_cd[]' id='item_cd_" + currentFlag + "'  placeholder='품목선택을 하시려면 클릭하세요' value='" + json[i].item_cd + "' readonly /></td>";
					tag += "<td><input type='text' class='form-control ' name='item_nm[]' id='item_nm_" + currentFlag + "' value='" + json[i].item_nm + "' readonly /></td>";
					tag += "<td><input type='text' class='form-control standard1' name='standard1[]' id='standard1_" + currentFlag + "' value='" + json[i].standard1 + "' readonly /></td>";
					tag += "<td><input type='text' class='form-control unit' name='unit[]' id='unit_" + currentFlag + "' value='" + json[i].unit + "'  /></td>";
					tag += "<td><input type='text' class='form-control material ' name='material[]' id='material_" + currentFlag + "'  onclick='this.select();' value='" + json[i].material + "' /></td>";
					tag += "<td><input type='text' class='form-control text-right cnt' name='cnt[]' id='cnt_" + currentFlag + "' onkeyup='calculation(" + currentFlag + ");input_comma(this);' onclick='this.select();' onblur='calculationTotal(" + currentFlag + ");' value='" + json[i].cnt + "' /></td>";
					tag += "<td><input type='text' class='form-control' name='remark[]' id='remark_" + currentFlag + "' onclick='this.select();' value='" + json[i].remark + "'/></td>";
					tag += "</tr>";
				...
				tag += "<tr><td colspan='10' align='center'>등록된 수리 품목이 없습니다.</td><tr>";
			}
