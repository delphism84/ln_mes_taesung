	...
	<div class="main-content-inner">
		<div class="page-content">
			<div class="row">
				<div class="col-xs-12">
					...
					<form name="frm" id="frm" method="post" action="index.php" enctype="multipart/form-data">
						...
						<input type="hidden" name="action" id="action" value="updateReleaseRequest" />
						...
						<table id="simple-table" class="table  table-bordered table-hover">
							<tr>
								<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 요청일자</th>
								<td class="col-xs-5">
									<div>
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input class=" date-picker" name="release_dt" id="release_dt" type="text" value="<?=$t->release_dt?>" data-date-format="yyyy/mm/dd" readonly/>
												<span class="input-group-addon">
													<i class="fa fa-calendar bigger-110"></i>
												</span>
												- <input  type="text" class="date-picker" name="release_cha" id="release_cha" style='width:30px' value="<?=$t->release_cha?>" data-date-format="yyyy/mm/dd" disabled/>
											</div>
										</span>
									</div>
								</td>
								...
								<td class="col-xs-5">
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input type="hidden" name="work_uid" id="work_uid" value="<?=$t->work_uid?>" readonly />
												<input type="text" name="work_cd" id="work_cd" value="<?=$t->work_cd?>"  readonly />
											</div>
										</span>
									</div>
								</td>
							...
							<tr>
							<!--
								<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 출고창고</th>
								<td class="col-xs-5">
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input type="hidden" name="warehouse_cd" id="warehouse_cd" value="<?=$t->warehouse_cd?>"/>
												<input type="text" name="warehouse_nm" id="warehouse_nm" value="<?=$t->warehouse_nm?>" onclick="centerOpenWindow('views/popup/warehouseList.php', '창고리스트', 400, 600)" />
							...
							-->
								<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 받는공장</th>
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											<input type="hidden" name="wh_cd_t_cd" id="wh_cd_t_cd" value="<?=$t->wh_cd_t_cd?>" />
											<input type="text" name="wh_cd_t_nm" id="wh_cd_t_nm" value="<?=$t->wh_cd_t_nm?>" onclick="centerOpenWindow('views/popup/warehouseToList.php', '생산공장', 400, 600)" />
											<span class="input-group-addon btn-purple" style="cursor:pointer" onclick="centerOpenWindow('views/popup/warehouseToList.php', '생산공장', 400, 600)">
												<i class="ace-icon fa fa-search icon-on-right bigger-110" style="color:#ffffff"></i>
											</span>
										</div>
									</span>
								...
								<td class="col-xs-5"><input type="file" name="attach" id="attach" value="<?=$t->attach?>"/></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<? if ($t->state == "2"){ ?>
									<select name="state" id="state"  style='width:200px;'>
										<option value="2" <? if($t->state == "2") echo "selected"; ?>>출고완료</option>
										<option value="3" <? if($t->state == "3") echo "selected"; ?>>출고취소</option>
									</select>
								<? }else if($t->state =="3"){ ?>
									<select name="state" id="state"  style='width:200px;'>
										<option value="3" <? if($t->state == "3") echo "selected"; ?>>출고취소</option>
						...
						<? if($t->state == "0"){ ?>
							<a class="btn btn-xs btn-success" onclick="centerOpenWindow('views/popup/releaseRequestItemList.php', '품목리스트', 800, 500)">품목선택</a>
						<? } ?>

						<table id="release_request_item" class="table  table-bordered table-hover" style="margin-top:10px">
							<thead>
								<tr>
									...
									<th class="center col-xs-1" style="background-color:#f1f1f1">출고 수량</th>
									<th class="center col-xs-2" style="background-color:#f1f1f1">LOT_NO</th>
									<th class="center col-xs-2" style="background-color:#f1f1f1">출고창고</th>
								</tr>
							</thead>
							<tbody>
								<tr>
			...
			<div class="clearfix form-actions center">
				...
				<? if($t->state != "3"){ ?>
					<button class="btn btn-info registBtn" type="button" onclick="formSubmit()">
...
function calculation(flag) {
	...
	var remain = $("#remain_cnt_"+flag).val();

	if(Number(removeComma(cnt)) > Number(removeComma(remain))){
		$("#cnt_" + flag).val(remain);
	}
...
function getWorkReleaseRequestItem(){
	var tag = "";
	var flag = $("#flag").val();
	var uid = $("#uid").val();
	var state = '<?=$t->state?>';

	$.getJSON("ajax/item.php",{"mode":"getWorkReleaseRequestItem","uid":uid, "state":state },
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					tag += "<input type='hidden' name='lot_no_nm[]' id='lot_no_nm_"+flag+"' value='" + json[i].lot_no_nm + "' />";
					tag += "<tr>";
					tag += "<td class='center'><i class='delBtn fa fa-minus-square fa-2x' aria-hidden='true' onclick='delTr(this)'></i></td>";
					tag += "<td><input type='text' class='form-control' name='item_cd[]' id='item_cd_"+flag+"' value='" + json[i].item_cd + "' readonly/></td>";
					...
					tag += "<td><input type='text' class='form-control cnt text-right' name='remain_cnt[]' id='remain_cnt_"+flag+"' value='" + json[i].remain_cnt + "' readonly/></td>";
					if(state == "0"){
						tag += "<td><input type='text' class='form-control cnt text-right' name='cnt[]' id='cnt_"+flag+"' value='" + json[i].cnt + "' onkeyup='calculation(" + flag + ");input_comma(this);' onclick='this.select();' onblur='calculationTotal(" + flag + ");' value='" + json[i].cnt + "'  /></td>";

						tag += "<td><input type='text' class='form-control lot_no' name='lot_no_cd[]' id='lot_no_cd_" + flag + "' onclick=\"lotnocdFlag(" + flag + ");lot_no_reg(" + flag + ",'"+ json[i].item_cd +"','"+ json[i].standard1 +"','"+ json[i].remain_cnt +"')\" value='"+ json[i].lot_no_cd +"'  placeholder='[click]' readonly /><input type='hidden' name='lot_no_nm[]'  id='lot_no_nm_" + flag + "' value='"+ json[i].lot_no_nm +"'/></td>";

						tag += "<td><input type='text' class='form-control lot_no' name='warehouse_nm[]' id='warehouse_nm_" + flag + "' onclick=\"lotnocdFlag(" + flag + ");lot_no_reg(" + flag + ",'"+ json[i].item_cd +"','"+ json[i].standard1 +"','"+ json[i].remain_cnt +"')\" value='"+ json[i].warehouse_nm +"' placeholder='[click]' readonly/><input type='hidden' name='warehouse_cd[]'  id='warehouse_cd_" + flag + "' value='"+ json[i].warehouse_cd +"'/></td>";


					}else{
						tag += "<td><input type='text' class='form-control cnt text-right' name='cnt[]' id='cnt_"+flag+"' value='" + json[i].cnt + "' onkeyup='calculation(" + flag + ");input_comma(this);' onclick='this.select();' onblur='calculationTotal(" + flag + ");' value='" + json[i].cnt + "'  readonly/></td>";

						
						tag += "<td><input type='text' class='form-control lot_no' name='lot_no_cd[]' id='lot_no_cd_" + flag + "'  value='"+ json[i].lot_no_cd +"'  placeholder='[click]' readonly /><input type='hidden' name='lot_no_nm[]'  id='lot_no_nm_" + flag + "' value='"+ json[i].lot_no_nm +"'/></td>";

						tag += "<td><input type='text' class='form-control lot_no' name='warehouse_nm[]' id='warehouse_nm_" + flag + "' value='"+ json[i].warehouse_nm +"' placeholder='[click]' readonly/><input type='hidden' name='warehouse_cd[]'  id='warehouse_cd_" + flag + "' value='"+ json[i].warehouse_cd +"' /></td>";
				...
				tag += "<tr><td colspan='9' align='center'>등록된 품목이 없습니다.</td><tr>";
			}
			$("#release_request_item tbody").html(tag);
			calculationTotal()
		}
	);	
}
...
///////////////////////////////////////////////
function lot_no_reg(cidx, item_cd , standard1 , remain_cnt){
	...
	var url = "index.php?controller=base&action=listLotNoPop2&idx="+cidx+"&item_cd=" + item_cd + "&standard1="+ standard1 +" &out='out'&pop=Y&dialogID=id-btn-dialog1";
...
//구매출고 내역에서 LOT_NO를 검색해서 선택하는 방식 erp_warehousing_lot_no 테이블 이용
...
function formSubmit(){
	...
	*/
	...
	if (state_before =="2" && state=="2"){
		var result = confirm('이미 출고 완료된 건 입니다.\n상태값을 변경하면 자재현황이 오류가 발생 합니다.\n그래도 변경하시겠습니까?'); 
		if(result) { 
			$("#frm").submit();
		}
	}else if (state_before =="2" && state=="3"){
		var result2 = confirm('출고 취소시 \n현재날짜로 재입고 처리됩니다.\n그래도 변경하시겠습니까?'); 
		if(result2) { 
			$("#frm").submit();
		}
	}else{
