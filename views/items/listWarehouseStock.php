							...
							<table id="item_list" class="table  table-bordered table-striped">
								<thead class="thin-border-bottom">
									<tr>
										...
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 총재고수량</th><th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> Lot_NO</th>
										<th class="col-xs-3"><i class="ace-icon fa fa-caret-right blue"></i> 창고이동</th>
									</tr>
								</thead>
								<tbody></tbody>
							</table>
		...
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "<td nowrap>" + json[i].standard1+ "</td>";
					tag += "<td>" + json[i].material + "</td>";
					tag += "<td>" + json[i].unit + "</td>";
					tag += "<td style='text-align:left;color:Coral;'><b>" + json[i].remain_cnt + "</b></td>";
					tag += "<td style='text-align:left'>" + json[i].lot_no + "</td>";
					tag += "<td><select name='warehouse_cd[]' id='warehouse_cd_" + i + "'>" + warehouse + "</select> <input type='text' class='onlynum' name='cnt[]' id='cnt_" + i + "' placeholder='이동수량' onkeyup=\"checkCnt("+ i +",'"+ json[i].remain_cnt +"')\" style='width:100px;' /> <input type='button' class='btn btn-xs btn-danger' value='창고이동' onclick=\"moveItem('" + json[i].warehouse_cd + "', '" + json[i].item_cd + "' , '" + json[i].standard1 + "', " + i + ",'" + json[i].lot_no + "')\" /></td>";
					tag += "</tr>";
				}
			}else{
...
function checkCnt( i , remain_cnt ){
	var remain_cnt = Number(remain_cnt.replace(/[^\d]+/g, ''));
	var cnt = Number($("#cnt_"+i).val());
	
	//alert(remain_cnt + "//" + cnt);
	if(remain_cnt < cnt){
		
		$("#cnt_"+i).val(remain_cnt);
	}
}


function moveItem(warehouse_cd, item_cd, standard, i , lot_no ){
	//alert(warehouse_cd+"//"+item_cd+"//"+standard+"//"+ i +"//" + lot_no );
	
	if(confirm("해당 품목을 창고이동 하시겠습니까?")) {
		var to = $("#warehouse_cd_" + i + " option:selected").val();
		var warehouse_nmTo = $("#warehouse_cd_" + i + " option:selected").text();
		var cnt = $("#cnt_" + i).val();
		var dataString = "mode=moveWarehouseItem2&from=" + warehouse_cd + "&to=" + to + "&item_cd=" + item_cd + "&standard1=" + standard + "&move_cnt=" + cnt + "&lot_no="+lot_no + "&warehouse_nmTo="+warehouse_nmTo;
		$.ajax({
			type : "post",
			data : dataString,
			url : "ajax/item.php",
			success : function(str) {
				if(str == "success") getStock( $("#warehouse_click").val() );
			}
		});
	}
}
