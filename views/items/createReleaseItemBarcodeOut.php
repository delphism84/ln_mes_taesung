			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form name="frm" id="frm" method="post" action="index.php" enctype="multipart/form-data">
						...
						<input type="hidden" name="action" id="action" value="outReleaseRequestBarcodeOut" />
						...
						<table id="simple-table" class="table  table-bordered table-hover">
							<tr>
								...
								<td class="col-xs-5"><input type="text" name="barcode" id="barcode" onchange="registOut()" class="form-control" placeholder="바코드를 스캔해 주세요"  /></td>
								<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right red"></i> 출고일자</th>
								<td class="col-xs-5">
									<div>
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input class=" date-picker" name="release_dt" id="release_dt" type="text" value=<?=date('Y/m/d')?> data-date-format="yyyy/mm/dd" readonly/>
							...
							<tr>
								...
								<td class="col-xs-5" colspan="3">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											<input type="hidden" name="wh_cd_t_cd" id="wh_cd_t_cd" />
											<input type="text" name="wh_cd_t_nm" id="wh_cd_t_nm"  onclick="centerOpenWindow('views/popup/warehouseToList.php', '생산공장', 400, 600)" readonly/>
						...
						<table id="release_request_item" class="table  table-bordered table-hover" style="margin-top:10px">
							<thead>
								<tr>
									...
									<th class="center col-xs-2" style="background-color:#f1f1f1">입고일</th>
								</tr>
							</thead>
							<tbody>
								<tr>
...
//바코드를 불러와서 postBarcodeReleaseItem 로 넘김
function registOut(){
	...
	//먼저 들어온 재고가 있는지 확인.
	var dataString = "mode=searchItemLot4&lot_no=" + barcode;
	$.ajax({
		type : "post",
		url : "ajax/lot_no.php",
		data : dataString,
		...
		success : function(str){
			...
			if(str.trim()=="no") alert("더빨리 입고된 lot가 있습니다.");
			else if(str.trim()=="yes") alert("가장 빠른 lot 입니다.");		
		}
	});


	var tag = "";
	$.getJSON("ajax/item.php",{"mode":"getBarcodeItem", "barcode" : barcode},
		function(json){
			if(json != null) {
				for(var i =0 ; i< json.length ; i++){

					postBarcodeReleaseItem( json[i].item_cd, json[i].item_nm, json[i].standard1,json[i].warehouse_cd , json[i].warehouse_nm , json[i].unit, json[i].remain_cnt , json[i].lot_no , json[i].in_date);
				}
			}else{
...
function postBarcodeReleaseItem( item_cd, item_nm, standard1, warehouse_cd , warehouse_nm , unit, remain_cnt , lot_no , in_date ) {
	var flag = $("#flag").val();
	<?if ($mode !="modify")	{?>
		if (flag=='1')
		{
			$("#release_request_item tbody").empty(); //최초 tbody 초기화
		}
	<?}?>
	var tag = "";
	...
	$.each($(".lot_no") , function () {
		lot.push($(this).val());
	});

	$.each($(".warehouse_cd") , function () {
		warehouse.push($(this).val());
	});

	for(var i = 0 ; i <= arr.length ; i++) {
		item.push( arr[i] + std1[i] + lot[i] + warehouse[i] );
	}
	
	var check = item_cd + standard1 + lot_no + warehouse_cd;
	...
	if(idx >= 0) {
		...
		tag += "<td><input type='hidden' class='form-control warehouse_cd' name='warehouse_cd[]' id='warehouse_cd_" + flag + "' value='" + warehouse_cd + "'  /><input type='text' class='form-control warehouse_nm' name='warehouse_nm[]' id='warehouse_nm_" + flag + "' value='" + warehouse_nm + "'  readonly/></td>";
		tag += "<td><input type='text' class='form-control remain_cnt text-right' name='remain_cnt[]' id='remain_cnt_" + flag + "' onkeyup='calculation(" + flag + ");input_comma(this);' onclick='this.select();' onkeyup='calculationTotal(" + flag + ");' value='" + remain_cnt + "' readonly/></td>";
		tag += "<td><input type='text' class='form-control text-right cnt' name='cnt[]' id='cnt_" + flag + "' onkeyup=\"calculation6(" + flag + ",'"+ remain_cnt +"')\";  value='' /></td>";
		//tag += "<td><input type='text' class='form-control lot_no' name='lot_no[]' id='lot_no_" + flag + "' onclick='lotnocdFlag(" + flag + ");lot_no_reg(" + flag + ")' value='"+ lot_no +"' readonly /></td>";
		tag += "<td><input type='text' class='form-control lot_no' name='lot_no[]' id='lot_no_" + flag + "' onclick='lotnocdFlag(" + flag + ");' value='"+ lot_no +"' readonly /></td>";
		tag += "<td><input type='text' class='form-control lot_no' name='in_date[]' id='in_date_" + flag + "' value='"+ in_date.substr(0,10) +"' readonly /></td>";
		tag += "</tr>";

		$("#release_request_item tbody").append(tag);
...
//재고수량보다 많이 출고 X
function calculation6( flag  , remain_cnt ){
	
	var cnt = Number($("#cnt_"+flag).val());
	
	if(Number(removeComma(remain_cnt)) < cnt){
...
function getWorkReleaseRequest(wid){
	var tag = "";
	var flag = $("#flag").val();

	$.getJSON("ajax/item.php",{"mode":"getWorkReleaseRequest","wid":wid},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "<td><input type='text' class='form-control' name='material[]' id='material_"+flag+"' value='" + json[i].material + "' /></td>";
					tag += "<td><input type='text' class='form-control' name='unit[]' id='unit_"+flag+"' value='" + json[i].unit + "' /></td>";
					tag += "<td><input type='text' class='form-control cnt text-right' name='remain_cnt[]' id='remain_cnt_"+flag+"' value='" + json[i].remain_cnt + "' /></td>";
					...
					tag += "<td><input type='text' class='form-control' name='lot_no_cd[]' id='lot_no_cd_"+flag+"' value='" + json[i].lot_no_cd + "' onclick=\"lotnocdFlag(" + flag + ");lot_no_search('" + flag + "','"+json[i].item_cd+"','"+json[i].standard1+"')\" readonly></td>";
					tag += "</tr>";
