...
<body>
    ...
    <div id="wrap_pop">
        ...
        <input type="hidden" id="hidHeightParam" name="hidHeightParam"  /> <!--종이 인쇄시 호환성일경우 Page Break시 hidHeightParam 지정된 높이를 사용함 -->      
					...
					<TD style="BORDER-BOTTOM: #000 1px solid; WORD-WRAP: break-word; BORDER-TOP: #000 1px solid; BORDER-RIGHT: #000 1px solid">&nbsp;<?=$t->consignment_dt?></TD></TR>
					<TR>
                    ...
                    <table class="p_rptC" style="width:650px;table-layout:fixed;word-break:break-all; word-wrap: break-word;">
						...
						if ($result){
							...
							while($rows=@mysql_fetch_object($result)) {
								?>	
						<tr style="height:20px;"><td class=' left font11px' style='overflow:hidden' ><?=$rows->item_nm?>[<?=$rows->standard1?>]</td><td class=' right font11px' style='overflow:hidden' ><?=$rows->cnt?></td><td class=' left font11px' style='overflow:hidden' ><?=$rows->remark?></td></tr>
						<?
						}
						}
						?>
