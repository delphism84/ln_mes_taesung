...
<!-- PAGE CONTENT BEGINS ---------------------------------------------------------------------------------------------------------------------------------------->
						...
						<div class="col-xs-6" style="float:right">
							...
							<div style="float:right">
								<select class="form-control" name="search_choice" id="search_choice">
									...
									<option value="standard">standard_cd</option>
					...
					<div class="widget-body">
						<div class="widget-main no-padding">
							<table id="standard_code_list" class="table table-bordered table-striped">
								<thead class="thin-border-bottom">
									<tr>
										...
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 규격코드</th>
										<th class="col-xs-2"><i class="ace-icon fa fa-caret-right blue"></i> 규격코드명</th>
										<th class="col-xs-2"><i class="ace-icon fa fa-caret-right blue"></i> 메모</th>
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 품목</th>
									</tr>
								</thead>
								<tbody></tbody>
							</table>
					...
					<div class="clearfix form-actions" style="margin-top:0px">
						<div class="col-md-12">
							...
							<div class="col-xs-6 right" style="text-align:right">
								<button class="btn btn-info" type="button" onclick="standard_code_reg('0','0')">
        ...
        <div class="modal-content" >
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">규격코드등록</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="standard_code_reg_frame" frameborder="0" width="99%" height="350" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
    ...
    <div class="modal-dialog modal-lg" style="width:30%">
        <div class="modal-content" >
            <div class="modal-header" id="modal-header2">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">규격코드수정</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="standard_code_modify_frame" frameborder="0" width="99%" height="350" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
<div id="id-btn-dialog3" class="modal fade" >
    <div class="modal-dialog modal-lg" style="width:70%">
        <div class="modal-content" >
            ...
            <div class="modal-body">
                <p><iframe src="" id="standard_code_print_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
function search(){
	...
	if(search_choice == "standard_cd") {
		$("#where").val(" where standard_cd like '%" + txt + "%' ");
	} else if(search_choice == "standard") {
		$("#where").val(" where standard like '%" + txt + "%' ");
	}
	getLotNo(1);
}
...
function getLotNo(page) {
	...
	$.getJSON("ajax/standard_code.php",{"page":page, "mode":"get_standard_code", "rpp" : rpp, "adjacents" : adjacents, "where":search},//, "search_choice" : search_choice, "txt" : txt},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "</td>";
					<?}?>
					tag += "<td><a href='#' onclick='standard_code_modiy(" + json[i].uid + ")'>" + json[i].standard + "</a></td>";
					tag += "<td><a href='#' onclick='standard_code_modiy(" + json[i].uid + ")'>" + json[i].standard_cd + "</a></td>";
					tag += "<td>" + json[i].standard_nm + "</td>";
					tag += "<td>" + json[i].etc + "</a></td>";
					tag += "<td><a href='javascript:void(0);'  onclick=\"postLotNo('" + json[i].standard + "','" + json[i].standard_cd + "')\"></a></td>";
					tag += "</tr>";
				}
			}

			$("#standard_code_list tbody").html(tag);

			var table = "erp_standard_code";
...
function deleteSelect(){
	if(confirm("선택하신 규격코드 정보를 삭제하시겠습니까? 다른 데이터와 연동된 품목 정보는 삭제되지 않습니다.")) {
		$(".chk").each(function(){
		...
		var dataString = "mode=deleteSelectStandard&table=erp_standard_code&uids=" + $("#check_uids").val();
		$.ajax({
			type : "post",
			url : "ajax/standard_code.php",
			data : dataString,
			async : false,
			success : function(str){
				if(str.trim() != "success") {
...
<!--
	function standard_code_reg(cidx, acd)
	{
	...
	var url = "index.php?controller=base&action=registStandardCode&idx="+cidx+"&pop=Y&aci_cd="+acd+"&dialogid=id-btn-dialog1";
	$("#standard_code_reg_frame").attr("src", url);
	}
			
	function standard_code_modiy(uid)
	{
	...
	var url = "index.php?controller=base&action=modifyStandardCode&uid="+uid+"&pop=Y&dialogid=id-btn-dialog2";
	$("#standard_code_modify_frame").attr("src", url);
	}
	
	function standard_code_print(uid)
	{
	...
	$("#standard_code_print_frame").attr("src", url);
	}
