...
<!-- PAGE CONTENT BEGINS ---------------------------------------------------------------------------------------------------------------------------------------->
					<div class="widget-header">
						<!-- <div class="col-xs-2" style="float:left">
							<select id="per" onchange="getLotNo(1)">
					...
					<div class="clearfix form-actions" style="margin-top:0px">
						<div class="clearfix col-xs-12" style="text-align:center"><span id="paging_area"></span></div>
						<div class="col-xs-12">
...
<input type="hidden" name="out" id="out" value="<?=$_GET['out']?>"/>
<input type="hidden" name="item_cd" id="item_cd" value="<?=$_GET['item_cd']?>"/>
<input type="hidden" name="standard1" id="standard1" value="<?=$_GET['standard1']?>"/>
...
function search(){
	...
	if(search_choice == "lot_no_cd") {
		$("#where").val(" and lot_no like '%" + txt + "%' ");
	} else if(search_choice == "item_cd") {
		$("#where").val(" and item_nm like '%" + txt + "%' ");
	}
	getLotNo(1);
}
...
function setItem(val) {
	...
	if(val == "all") {
		...
		$("#where").val(" where item_gb='" + val + "' order by regdate desc");
	}
	getLotNo(1);
}
...
function getLotNo(page) {
	...
	$.getJSON("ajax/lot_no.php",{"page":page, "mode":"get_lot_no2", "rpp" : rpp, "adjacents" : adjacents, "where":search , "item_cd" : $("#item_cd").val() , "standard1" : $("#standard1").val() },
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					<?}?>
					*/
					tag += "<td><a href='javascript:void(0);'  onclick=\"postLotNo('" + json[i].lot_no_cd + "','" + json[i].remain_cnt + "','" + json[i].warehouse_nm + "','" + json[i].warehouse_cd + "','" + json[i].work_cd + "')\">" + json[i].lot_no_cd + "</a></td>";
					//tag += "<td><a href='javascript:void(0);'  onclick=\"postLotNo('" + json[i].lot_no_cd + "','" + json[i].lot_no_nm + "','" + json[i].remain_cnt + "')\">" + json[i].lot_no_nm + "</a></td>";
					tag += "<td><a href='javascript:void(0);'  onclick=\"postLotNo('" + json[i].lot_no_cd + "','" + json[i].remain_cnt + "','" + json[i].warehouse_nm + "','" + json[i].warehouse_cd + "','" + json[i].work_cd + "')\">" + json[i].item_cd + "</td>";
					tag += "<td><a href='javascript:void(0);'  onclick=\"postLotNo('" + json[i].lot_no_cd + "','" + json[i].remain_cnt + "','" + json[i].warehouse_nm + "','" + json[i].warehouse_cd + "','" + json[i].work_cd + "')\">" + json[i].standard1 + "</td>";
				...
				tag += "<tr>";
				tag += "<td colspan='7' class='text-center'>등록된 로트No 가 없습니다.</td>";
				tag += "</tr>";
			}
			...
			if(lot_no_cd == "" || lot_no_cd == "all") {
				...
				var where = $("#where").val() + " and lot_no_cd='"  + txt + "'";
				//var where = $("#where").val();
			}
			*/
			//alert(search_choice)
			if(search_choice == "lot_no_cd" && txt !="") {
				where = $("#where").val(" where lot_no_cd like '%" + txt + "%'");
			} else if(search_choice == "item_cd" && txt !="") {
				where = $("#where").val(" where item_cd like '%" + txt + "%'");
			}
...
function postLotNo( lnc , remain_cnt ,warehouse_nm , warehouse_cd, work_cd ){
		if( remain_cnt <= 0 ){
			alert("재고가 없습니다.");
			return false;
		}
		...
		//먼저 들어온 재고가 있는지 확인.
		var dataString = "mode=searchItemLot2&item_cd="+ $("#item_cd").val() + "&standard1=" + $("#standard1").val()+ "&lot_no=" + lnc;
		$.ajax({
			type : "post",
			url : "ajax/lot_no.php",
			data : dataString,
	...
	$.each(window.parent.$(".lot_no") , function () {
		arr1.push($(this).val());
	});
	$.each(window.parent.$(".work_cd") , function () {
		arr2.push($(this).val());
	});

	for(var i = 0 ; i <= arr1.length ; i++) {
		lotno.push( arr1[i] + "/" + arr2[i] );
	}
	var check = lnc + "/" + work_cd;
	...
	if(idx >= 0) {
		...
		window.parent.$("#remain_cnt_"+flag).val(remain_cnt);	//부모창 재고수량 변경
		window.parent.$("#cnt_"+flag).val("");			//부모창 출하수량 0초기화
		window.parent.$("#box_cnt_"+flag).val(0);		//부모창 박스수량 0초기화

		window.parent.$("#warehouse_nm_"+flag).val(warehouse_nm);	//부모창  출고 창고입력
		window.parent.$("#warehouse_cd_"+flag).val(warehouse_cd);	//부모창  출고 창고입력
		window.parent.$("#work_cd_"+flag).val(work_cd);

		window.parent.$(".cntTotal").html("");				//부모창  출하갯수 total =0
