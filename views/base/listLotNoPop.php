					...
					<div class="widget-header">
						...
						<div class="col-xs-12" style="float:right">
							...
							<div style="float:right" class="col-xs-3">
								<select class="form-control" name="search_choice" id="search_choice" style='width:100px;'>
							...
							<table id="lot_no_list" class="table table-bordered table-striped">
								<thead class="thin-border-bottom">
									<tr>
										...
										<th class="col-xs-2"><i class="ace-icon fa fa-caret-right blue"></i> 항목명</th>
										<th class="col-xs-2"><i class="ace-icon fa fa-caret-right blue"></i> 유효기간</th>
...
function getLotNo(page) {
	...
	$.getJSON("ajax/lot_no.php",{"page":page, "mode":"get_lot_no", "rpp" : rpp, "adjacents" : adjacents, "where":search },
		function(json){
			if(json != null) {
...
function postLotNo(lnc,lnn){
		//이미 사용된 로트인지 확인
		var usedLot ="false"; 
		var dataString = "mode=LotCheck&lot_no=" + lnc;
		$.ajax({
			type : "post",
			url : "ajax/lot_no.php",
			data : dataString,
			...
			success : function(str){
				if(str.trim() == "success"){
					alert("입고에 사용가능한 LOT 입니다.");
					 usedLot = "false"; 
				}else{
					alert("입고에 이미 사용한 LOT 입니다.");
					 usedLot ="true"; 
				}
			}
		});
	...
	if( usedLot == "false"){
			...
			if( $("#out").val() != "" ){	
				//재고가 남아있는지 확인.후 lot no와 in out 비교하여 먼저 들어온 로트 자재가 있는지 확인시켜줌.
