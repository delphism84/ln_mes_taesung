			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form name="frm" id="frm" method="post" action="index.php" enctype="multipart/form-data">
						...
						<input type="hidden" name="action" id="action" value="inputWork" />
						...
						<table id="simple-table" class="table table-bordered table-hover">
							<tr>
								...
								<td class="col-xs-5" colspan="3">
									<input type="text" name="work_dt" id="work_dt" value="<?=date('Y/m/d')?>" <? if($_SESSION['auto_code'] == "y") echo "readonly"; ?> />
								</td>
							</tr>
							<tr>
								...
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											<input class=" date-picker" type="text" data-date-format="yyyy-mm-dd" name="work_start_dt" id="work_start_dt" />
							...
							<tr>
								...
								<tr>
									...
									<td><input type="text" name="workplan_cd" id="workplan_cd" /></td>
								</tr>
							</thead>
							<tbody></tbody>
						</table>
						...
						<table id="bom_list" class="table  table-bordered table-hover" style="margin-top:10px">
							<thead>
								<tr>
									<th style="background-color:#f1f1f1" colspan="6">만들어야 하는 제품</th>
								</tr>
								<tr>
									...
									<th style="background-color:#f1f1f1">생산해야할 수량</th>
								</tr>
							</thead>
							<tbody></tbody>
						</table>

						<a class="btn btn-xs btn-success" onclick="centerOpenWindow('views/popup/workItemList.php', '품목 리스트', 800, 500)">품목선택</a>
						<a class="btn btn-xs btn-info" onclick="centerOpenWindow('views/popup/planList.php', '생산계획서 리스트', 800, 500)"">생산계획서 품목가져오기</a>
						<table id="product" class="table table-bordered table-hover" style="margin-top:10px">
							<thead>
								<tr>
									...
									<th class="center col-xs-1" style="background-color:#f1f1f1">소요량</th>
	...
	$.getJSON("ajax/production.php",{"mode":"getWorkplanBom","workplan_cd":workplan_cd},
		function(json){
			if(json != null) {
				...
				for(var i = 0 ; i < json.length ; i++){
					checkCalBom(json[i].item_cd, json[i].item_nm, json[i].standard1, json[i].unit, json[i].cnt, json[i].current_cnt);
				}
			}
		}
	);	
...
function checkCalBom(item_cd, item_nm, standard1, unit, cnt, current_cnt){
	var tag = "";
