	...
	<div class="main-content-inner">
		...
		<div class="breadcrumbs ace-save-state" id="breadcrumbs">
			<ul class="breadcrumb">
				...
				<li class="active">생산실적관리( 검사 / 포장 )</li>
			</ul>
		</div>
		<!-- // 페이지 상단 Location -->
							...
							<table id="product_perf_repost_list" class="table  table-bordered table-striped">
								<thead class="thin-border-bottom">
									<tr>
										...
										<?}?>
										<th><i class="ace-icon fa fa-caret-right blue"></i> 포장일자</th>
										<th><i class="ace-icon fa fa-caret-right blue"></i> 출하일자</th>
										<th><i class="ace-icon fa fa-caret-right blue"></i> 작업지시번호</th>
										...
										<th><i class="ace-icon fa fa-caret-right blue"></i> 포장수량</th>
        ...
        <div class="modal-content" >
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">실적처리등록(포장)</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="product_perf_report_reg_frame" frameborder="0" width="99%" height="550" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
    ...
    <div class="modal-dialog modal-lg" style="width:70%">
        <div class="modal-content" >
            <div class="modal-header" id="modal-header2">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">실적처리수정(포장)</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="product_perf_report_modify_frame" frameborder="0" width="99%" height="550" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
<div id="id-btn-dialog3" class="modal fade" >
    <div class="modal-dialog modal-lg" style="width:70%">
        <div class="modal-content" >
            ...
            <div class="modal-body">
                <p><iframe src="" id="product_perf_report_print_frame" frameborder="0" width="99%" height="550" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
	...
	$.getJSON("ajax/production.php",{"page":page, "mode":"getProductPerfReports", "rpp" : rpp, "adjacents" : adjacents, "where" : where, "gubun" : gubun},
		function(json){
			if(json != null) {
				...
				tag += `<tr><td colspan='10'>합계</td><td class='text-right'>${comma(totalProductQty)}</td><td class='text-right'>${comma(totalFaultQty2)}</td><td class='text-right'>${comma(totalFaultQty3)}</td><td class='text-right'>${comma(totalFaultQty4)}</td><td class='text-right'>${comma(totalFaultQty5)}</td><td class='text-right'>${comma(totalFaultQty6)}</td><td class='text-right'>${comma(totalFaultQty7)}</td><td colspan='2'></td></tr>`;
			}else{
				tag += "<tr>";
				tag += "<td colspan='13' class='center' style='height:20px'> 등록된 제품 포장 실적 데이터가 없습니다. </td>";
				tag += "</tr>";
			}
...
<script type="text/javascript">
	...
	var url = "index.php?controller=production&action=registPagePPReportsPacking&idx="+cidx+"&pop=Y&aci_cd="+acd+"&dialogID=id-btn-dialog1";
	...
	var url = "index.php?controller=production&action=modifyPagePPReportsPacking&pop=Y&uid="+uid+"&dialogID=id-btn-dialog2";
...
function openWin(uid){  
	...
	window.open("views/production/printProductPerfReportsPacking.php?uid="+uid, "실적처리", "width=1024px, height=900, toolbar=no, menubar=no, scrollbars=yes, resizable=no" );
}  
//-->  
</script>  
