					...
					<div class="clearfix form-actions" style="margin-top:0px">
						<div class="col-md-12">
							...
							<div class="col-xs-6 right" style="text-align:right">
								<button class="btn btn-info" type="button" onclick="productOutput_reg('0','0')">
        ...
        <div class="modal-content" >
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">생산실적관리등록</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="product_output_reg_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
    ...
    <div class="modal-dialog modal-lg" style="width:70%">
        <div class="modal-content" >
            <div class="modal-header" id="modal-header2">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">생산실적관리수정</h4>
		...
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "</td>";
					<?}?>
					tag += "<td><a href='javascript:void(0);'  onclick='productOutput_modiy(" + json[i].uid +")' >" + json[i].production_dt + "</a></td>";
					tag += "<td>" + json[i].day_gubun + "</a></td>";
					tag += "<td>" + json[i].emp_nm + "</a></td>";
					tag += "<td>" + json[i].p_plan_tm + "</a></td>";
					tag += "<td><a href='javascript:void(0);'  onclick='productOutput_modiy(" + json[i].uid +")' >"+ json[i].item_cd + "</a></td>";
					tag += "<td><a href='javascript:void(0);'  onclick='productOutput_modiy(" + json[i].uid +")' >"+ json[i].item_nm + "</a></td>";
				...
				tag += "<tr>";
				tag += "<td colspan='13' class='center' style='height:20px'> 등록된 실적데이터가 없습니다. </td>";
				tag += "</tr>";
			}
...
<script type="text/javascript">
<!--
	function productOutput_reg(cidx, acd)
	{
	...
	var url = "index.php?controller=production&action=registpageProductOutput&idx="+cidx+"&pop=Y&aci_cd="+acd+"&dialogID=id-btn-dialog1";
	$("#product_output_reg_frame").attr("src", url);
	}
			
	function productOutput_modiy(uid)
	{
	...
	var url = "index.php?controller=production&action=modifypageProductOutput&pop=Y&uid="+uid+"&dialogID=id-btn-dialog2";
	$("#product_output_modify_frame").attr("src", url);
	}
	
	function ProductOutput_print(uid)
	{
