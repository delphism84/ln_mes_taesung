							...
							<table id="work_list" class="table  table-bordered table-striped">
								<thead>
									<tr>
										...
										<?}?>
										<th class="col-xs-1" nowrap><i class="ace-icon fa fa-caret-right blue"></i> 작업지시번호</th>
					...
					<div class="clearfix form-actions" style="margin-top:0px">
						<div class="col-md-12">
							...
							<div class="col-xs-6 right" style="text-align:right">
								<button class="btn btn-info" type="button" onclick="window.parent.closeModal('id-btn-dialog4');">
...
<input type="hidden" name="work_cd" id="work_cd" value="<?=$work_cd?>" />
...
function getWorkView(page){
	...
	//alert(work_cd)
	$.getJSON("ajax/production.php",{"page":page, "mode":"getWorkView", "rpp" : rpp, "adjacents" : adjacents, "work_cd" : work_cd},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "</td>";
					<?}?>
					tag += "<td><a href='#' onclick=\"wopen('" + json[i].uid + "')\">" + json[i].work_cd + "</a></td>";
					...
					tag += "<td class='text-right'>" + json[i].order_cnt + "</td>";
