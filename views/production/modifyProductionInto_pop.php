			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form name="frm" id="frm" method="post" action="index.php" enctype="multipart/form-data">
						...
						<input type="hidden" name="action" id="action" value="updatePageProductionInto" />
						...
						<table id="simple-table" class="table table-bordered table-hover">
							<tr>
									...
									<div>
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input class=" date-picker" name="p_into_dt" id="p_into_dt" type="text" value="<?=$t->p_into_dt?>" data-date-format="yyyy/mm/dd" readonly/>
								...
								<th class="col-xs-1" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right blue"></i> 작업지지서</th>
								<td class="col-xs-5">
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input type="hidden" name="work_uid" id="work_uid" value="<?=$t->work_uid?>"/>
												<input type="text" name="work_cd" id="work_cd" value="<?=$t->work_cd?>" onclick="centerOpenWindow('views/popup/workProductionIntoListPut.php', '작업지시서 리스트', 800, 500)" readonly/>
							...
							<tr>
								...
								<td class="col-xs-5">
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input type="hidden" name="wh_cd_f_cd" id="wh_cd_f_cd" value="<?=$t->wh_cd_f_cd?>"/>
													<input type="text" name="wh_cd_f_nm" id="wh_cd_f_nm" value="<?=$t->wh_cd_f_nm?>"  readonly/>
													<!--
												<input type="text" name="wh_cd_f_nm" id="wh_cd_f_nm" value="<?=$t->wh_cd_f_nm?>" onclick="centerOpenWindow('views/popup/warehouseFromList.php', '생산공장', 400, 600)" />-->
								...
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											...
											<!--
											<input type="text" name="warehouse_nm" id="warehouse_nm" value="<?=$t->warehouse_nm?>"onclick="centerOpenWindow('views/popup/warehouseList.php', '입고창고', 400, 600)" />-->
											<span class="input-group-addon btn-purple" style="cursor:pointer">
												<i class="ace-icon fa fa-search icon-on-right bigger-110" style="color:#ffffff"></i>
											</span>
										</div>
								...
								<th class="col-xs-1" style="background-color:#f1f1f1">프로젝트</th>
									<td class="col-xs-5">
										<div class="input-group">
											<span class="input-icon input-icon-right">
												<div class="input-group">
													<input type="hidden" name="project_cd" id="project_cd" value="<?=$t->warehouse_cd?>" project_cd />
									...
									<div class="input-group">
											<span class="input-icon input-icon-right">
												<div class="input-group">
													...
													<input type="text" name="manager" id="emp_nm" value="<?=$t->emp_nm?>" onclick="centerOpenWindow('views/popup/employeeList.php', '담당자', 500, 500)" readonly />
							...
							<tr>
								...
								<td class="col-xs-11" colspan="3">
									<textarea id="remark" name="remark" class="form-control"><?=$t->remark?></textarea>
								</td>
							</tr>
						</table>
						<div>
						...
						<table id="work_list" class="table  table-bordered table-hover" style="margin-top:10px">
							<thead>
								<tr>
									<th class="center col-xs-1" style="background-color:#f1f1f1">작업지시서</th>	
							...
							<tbody>
								<!-- <tr>
									<td colspan='11' class='center'>등록된 작업지시서가 없습니다.</td>
								</tr> -->
							</tbody>
						</table>
						<!--
						...
						<table id="work_item_list" class="table table-bordered table-hover" style="margin-top:10px">
							<thead>
								<tr>
									...
									<!--<th class="center col-xs-1" style="background-color:#f1f1f1">추가수량</th>-->
...
function getWorkOrder(){
	...
	$.getJSON("ajax/production.php",{"mode":"getWorkOrder", "uid" : uid}, function(json){
		...
		getProductionIntoItem();//작업지지서 세부내용
	});
}
	...
	//생산입고 아이템 리스트 가져오기
function getProductionIntoItem(){
	var tag = "";
	var uid = $("#uid").val();

	var currentFlag = $("#flag").val();

	$.getJSON("ajax/production.php",{"mode":"getProductionIntoItem", "uid" : uid},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					//tag += "<td><input type='text' class='form-control id-btn-dialog1 warehouse_f_nm' name='warehouse_f_nm[]' id='warehouse_f_nm_" + currentFlag + "' onclick='warehouse_f_Flag(" + currentFlag + ")' value='" + json[i].warehouse_f_nm + "' readonly /></td>";
					//tag += "<td><input type='text' class='form-control id-btn-dialog2 warehouse_t_nm' name='warehouse_t_nm[]' id='warehouse_t_nm_" + currentFlag + "' onclick='warehouse_t_Flag(" + currentFlag + ")' value='" + json[i].warehouse_t_nm + "' readonly /></td>";
					//tag += "<td><input type='text' class='form-control addcnt text-right' name='addcnt[]' id='addcnt_" + currentFlag + "' onkeyup='input_comma(this);' onclick='this.select();' onblur='calculationTotal(" + currentFlag + ");' value='" + json[i].addcnt + "'/></td>";
					
					tag += "<td><input type='text' class='form-control cnt text-right' name='cnt[]' id='cnt_" + currentFlag + "' onkeyup='input_comma(this);' onclick='this.select();' onblur='calculationTotal(" + currentFlag + ");' value='" + json[i].cnt + "' readonly/></td>";
					...
					tag += "<td><input type='text' class='form-control' name='lot_no[]' id='lot_no_" + currentFlag + "'  onkeyup='checkLot(this.value); lotFlag("+currentFlag+")'  value='"+ json[i].lot_no+"' readonly/></td>";
					tag += "<td><span class=form-control' class='lotCheck'  id='lotCheck_" + currentFlag + "' style='color:green;'>-</span><input type='hidden' class='lotCheck2' id='lotCheck2_" + currentFlag + "'  value='' /></td>";
