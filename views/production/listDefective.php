				...
				<div class="col-xs-12">
					<div class="widget-header">
						<div class="col-xs-6" style="float:left">
							<select id="per" onchange="getDefective(1)">
					...
					<div class="widget-body">
						<div class="widget-main no-padding">
							<table id="defective_list" class="table  table-bordered table-striped">
								<thead class="thin-border-bottom">
									<tr>
										...
										<?}?>
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 작업지시서코드</th>
										...
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 규격</th>
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 규격</th>
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 불량사유</th>
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 생산지시수량</th>
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 불량수량</th>
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 불량률</th>
									</tr>
								</thead>
								<tbody></tbody>
							</table>
...
<input type="hideen" name="where" id="where" value=" group by item_cd,standard" />
...
function getDefective(page){
	...
	$.getJSON("ajax/production.php",{"page":page, "mode":"getDefective", "rpp" : rpp, "adjacents" : adjacents, "where" : where},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "</td>";
					<?}?>
					tag += "<td><a href='index.php?controller=sales&action=modifyPageEstimate&uid=" + json[i].uid + "'>" + json[i].work_cd + "</a></td>";
					...
					tag += "<td>" + json[i].percent_defective + " %</td>";
					tag += "</tr>";
				}
			}else{
				tag += "<tr>";
				tag += "<td colspan='12' class='center' style='height:20px'> 등록된 불량관리 리스트가 없습니다. </td>";
				tag += "</tr>";
			}

			$("#defective_list tbody").html(tag);

			var table = "erp_defective";
...
function deleteSelect(){
	if(confirm("선택하신 불량 정보를 삭제하시겠습니까?")) {
		$(".chk").each(function(){
		...
		var dataString = "mode=deleteSelectDefective&table=erp_estimate&uids=" + $("#check_uids").val();
		$.ajax({
			type : "post",
