							...
							<table id="product_perf_repost_list" class="table  table-bordered table-striped">
								<thead class="thin-border-bottom">
									<tr>
										...
										<?}?>
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 세척일자</th>
										...
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 세척수량</th>
        ...
        <div class="modal-content" >
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">실적처리등록(세척)</h4>
    ...
    <div class="modal-dialog modal-lg" style="width:70%">
        <div class="modal-content" >
            <div class="modal-header" id="modal-header2">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">실적처리수정(세척)</h4>
	...
	$.getJSON("ajax/production.php",{"page":page, "mode":"getProductPerfReports", "rpp" : rpp, "adjacents" : adjacents, "where" : where, "gubun" : gubun},
		function(json){
			if(json != null) {
				...
				tag += `<tr><td colspan='9'>합계</td><td class='text-right'>${comma(totalProductQty)}</td><td class='text-right'>${comma(totalFaultQty)}</td><td colspan='2'></td></tr>`;
			}else{
				tag += "<tr>";
				tag += "<td colspan='13' class='center' style='height:20px'> 등록된 세척 실적데이터가 없습니다. </td>";
				tag += "</tr>";
			}
...
<!--
	...
	var url = "index.php?controller=production&action=registPagePPReportsClean&idx="+cidx+"&pop=Y&aci_cd="+acd+"&dialogID=id-btn-dialog1";
	...
	var url = "index.php?controller=production&action=modifyPagePPReportsClean&pop=Y&uid="+uid+"&dialogID=id-btn-dialog2";
...
function openWin(uid){  
	...
	window.open("views/production/printProductPerfReportsClean.php?uid="+uid, "실적처리", "width=1024px, height=900, toolbar=no, menubar=no, scrollbars=yes, resizable=no" ); 
}  
//-->  
</script>  
