...
<!-- PAGE CONTENT BEGINS ---------------------------------------------------------------------------------------------------------------------------------------->
					...
					<div class="widget-header">
						<div class="col-xs-6" style="float:left">
							<select id="per" onchange="getProductInto(1)">
					...
					<div class="widget-body">
						<div class="widget-main no-padding">
							<table id="product_into_list" class="table  table-bordered table-striped">
								<thead>
									<tr>
										...
										<th class="col-xs-2" nowrap><i class="ace-icon fa fa-caret-right blue"></i> 인쇄</th>
									</tr>
								</thead>
								<tbody></tbody>
							</table>
					...
					<div class="clearfix form-actions" style="margin-top:0px">
						<div class="col-md-12">
							...
							<div class="col-xs-6 right" style="text-align:right">
								<button class="btn btn-info" type="button" onclick="product_into_reg('0','0')">
        ...
        <div class="modal-content" >
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">생산입고등록</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="product_into_reg_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
    ...
    <div class="modal-dialog modal-lg" style="width:70%">
        <div class="modal-content" >
            <div class="modal-header" id="modal-header2">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">생산입고</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="product_into_modify_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
<div id="id-btn-dialog3" class="modal fade" >
    <div class="modal-dialog modal-lg" style="width:70%">
        <div class="modal-content" >
            <div class="modal-header" id="modal-header2">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">생산입고인쇄</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="product_into_print_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
function getProductInto(page){
	...
	/*
	<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 입고번호</th>
	<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 생산입고</th>
	<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 생산된공장명</th>
	<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 받는창고명</th>
	<th class="col-xs-2"><i class="ace-icon fa fa-caret-right blue"></i> 품목</th>
	<th class="col-xs-2"><i class="ace-icon fa fa-caret-right blue"></i> 규격</th>
	<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 수량</th>
	<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 인쇄</th>
	*/
	$.getJSON("ajax/production.php",{"page":page, "mode":"getProductInto", "rpp" : rpp, "adjacents" : adjacents, "where" : where},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "</td>";
					<?}?>
					tag += "<td><a href='javascript:void(0);'  onclick='product_into_modiy(" + json[i].uid +")' >" + json[i].p_into_cd + "</a></td>";
					tag += "<td>" + json[i].work_cd + "</td>";
					tag += "<td>" + json[i].wh_cd_f_nm + "</td>";
					tag += "<td>" + json[i].warehouse_nm + "</td>";
					tag += "<td>" + json[i].item_nm + "</td>";
					tag += "<td>" + json[i].standard1 + "</td>";
					tag += "<td>" + json[i].unit + "</td>";
					tag += "<td class='text-right'>" + json[i].cnt + "</td>";
					tag += "<td class='center' nowrap><input type='button' class='btn btn-xs btn-success' value='인쇄' onclick='workProductionInto_Print("+ json[i].uid +")'/>&nbsp;<input type='button' class='btn btn-xs ' value='바코드' onclick='workProductionInto_barcode_Print("+ json[i].uid +")'/></td>";
					...
					tag += "<tr>";
					tag += "<td colspan='11' class='center'>등록된 생산입고가 없습니다.</td>";
					tag += "</tr>";
			}

			$("#product_into_list tbody").html(tag);

			var table = "erp_production_into";
...
function deleteSelect(){
	if(confirm("선택하신 생산입고 정보를 삭제하시겠습니까?")) {
		...
		var dataString = "mode=deleteSelectProduction&table=erp_estimate&uids=" + $("#check_uids").val();
		$.ajax({
			type : "post",
...
function registAllIn(uid, cnt) {
	if(confirm("전량 생산입고 처리 하시겠습니까?")) {
		...
		$.ajax({
			...
			url : "ajax/production.php",
			success : function(str) {
				if(str == "success") getProductInto(1);
			}
		});
	}
}
...
function workProductionInto_Print(p_cd)
{
	var param	= "?uid="+p_cd;
	var url		= '';
		url		= "../../views/doc_form/workProduction_into_print.php" + param;
	var popwin	= window.open(url, 'productionInto_print', 'width=1000, height=900, scrollbars=yes, resizable=yes, toolbar=no, menubar=no');
}
function workProductionInto_barcode_Print(p_cd)
{
	var param	= "?uid="+p_cd;
	var url		= '';
		url		= "../../views/doc_form/workProduction_into_barcode_print.php" + param;
	var popwin	= window.open(url, 'productionInto_print', 'width=1000, height=900, scrollbars=yes, resizable=yes, toolbar=no, menubar=no');
}
</script>
...
<script type="text/javascript">
<!--
	function product_into_reg(cidx, acd)
	{
	...
	var url = "index.php?controller=production&action=registPageProductionIntoPop&idx="+cidx+"&pop=Y&aci_cd="+acd+"&dialogID=id-btn-dialog1";
	$("#product_into_reg_frame").attr("src", url);
	}
			
	function product_into_modiy(uid)
	{
	...
	var url = "index.php?controller=production&action=modifyPageProductionIntoPop&pop=Y&uid="+uid+"&dialogID=id-btn-dialog2";
	$("#product_into_modify_frame").attr("src", url);
	}
	
	function product_into_print(uid)
	{
	...
	$("#product_into_print_frame").attr("src", url);
	}

	function close_popup()
	{	
		$.modal.close();
		$("#product_into_reg_frame").attr("src", "about:blank");
	}
	function closePopup()
	{
...
function openWin(uid){  
    //window.open("index.php?controller=production&action=productPerfReportsPrint&pop=Y&uid="+uid+"&dialogID=id-btn-dialog3", "생산입고", "width=600, height=900, toolbar=no, menubar=no, scrollbars=no, resizable=yes" ); 
	window.open("views/production/printProductPerfReports.php?uid="+uid, "생산입고", "width=1024px, height=900, toolbar=no, menubar=no, scrollbars=yes, resizable=no" ); 
}  
//-->  
</script>  
...
function openWinPrint(uid){  
    window.open("/views/accounting/doc_form/print/product_into_print.php?uid="+uid, "전표인쇄", "width=800, height=700, toolbar=no, menubar=no, scrollbars=yes, resizable=yes" );  
}  
//-->  
</script> 