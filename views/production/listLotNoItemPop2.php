							...
							<table id="lot_no_list" class="table table-bordered table-striped">
								<thead class="thin-border-bottom">
									<tr>
										...
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 투입대기</th>
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 입고일</th>
										<th class="col-xs-1"><i class="ace-icon fa fa-caret-right blue"></i> 관리</th>
									</tr>
								</thead>
								<tbody></tbody>
							</table>
...
function getLotNo(page) {
	...
	$.getJSON("ajax/lot_no.php",{"page":page, "mode":"get_input_ready", "rpp" : rpp, "adjacents" : adjacents, "where":search, "search_choice" : search_choice, "txt" : txt, "item_cd" : item_cd, "standard" : standard},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "</td>";
					<?}?>
					tag += "<td><a href='javascript:void(0);'  onclick=\"postLotNo('" + json[i].uid + "','" + json[i].lot_no_cd + "','" + json[i].item_nm + "','" + json[i].warehousing_dt + "')\">" + json[i].lot_no_cd + "</a></td>";
					tag += "<td><a href='javascript:void(0);'  onclick=\"postLotNo('" + json[i].uid + "','" + json[i].lot_no_cd + "','" + json[i].item_nm + "','" + json[i].warehousing_dt + "')\">" + json[i].item_nm + "</a></td>";
					...
					tag += "<td><input type='button' value='투입완료' onclick=\"inputReadyDel("+ json[i].uid +")\"></td>";
					tag += "</tr>";
				}
			}else{
				tag += "<tr>";
			...
			var table = "erp_release_input_ready";
			//if(lot_no_cd == "" ) {
			//	var where = "";
			//} else {
...
function inputReadyDel( uid ){
	
	var dataString = "mode=input_readyDel&uid=" + uid;

	$.ajax({
		type : "post",
		url : "ajax/input_readyDel.php",
		data : dataString,
...
function postLotNo( uid, lnc,lnn,dt){
	...
	if(idx >= 0) {
		...
		window.parent.$("#input_uid_" + flag).val(uid);
