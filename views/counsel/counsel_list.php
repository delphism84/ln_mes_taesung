					...
					<table id="counsel_list" class="table  table-bordered table-hover">
						<thead>
							<tr>
								...
								<th class="col-xs-1 center" style="background-color:#f1f1f1">상담일시</th>
								<th class="col-xs-2 center" style="background-color:#f1f1f1">상담유형</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">호응도</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">다음 진행 프로세스</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">다음 약속일</th>
							</tr>
						</thead>
						<tbody></tbody>
					</table>
...
function get_counsel(page){
	...
	$.getJSON("ajax/counsel.php",{"page":page, "mode":"get_counsel", "where":search, "account_type":account_type},
		function(json){
			if(json != null) {
				...
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "</td>";
					tag += "<td><a href='index.php?controller=counsel&action=counsel_view&uid=" + json[i].uid + "'>" + json[i].name + "</a></td>";
					tag += "<td>" + json[i].counsel_date + "</td>";
					tag += "<td>" + json[i].process + "</td>";
					tag += "<td>" + json[i].response + "</td>";
					tag += "<td>" + json[i].next_process + "</td>";
					tag += "<td>" + json[i].next_counsel_date + " " + json[i].next_counsel_time + "</td>";
					tag += "</tr>";
				}
			} 
...
function delete_select(){
	if(confirm("선택하신 거래처 정보를 삭제하시겠습니까?")) {
		...
		var dataString = "mode=delete_data&table=counsel&uids=" + $("#chk_uids").val();
		$.ajax({
			type : "post",
			url : "ajax/ajax.php",
			data : dataString,
