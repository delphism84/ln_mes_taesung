...
function setPage(page){
	...
	location.href = "./workProductionIntoListPut.php?page=" + page + "&search_txt=" + search_txt + "&state=" + $("#state option:selected").val();
}
...
function postWork(work_cd, item_cd, item_nm, standard1, material, unit, cnt, uid) {
	var flag = $(opener.document).find("#flag").val();
	if (flag=='1'){
		$(opener.document).find("#work_list tbody").empty(); //최초 tbody 초기화
		$(opener.document).find("#work_item_list tbody").empty(); //최초 tbody 초기화
	}
	...
	$("#work_uid",opener.document).val(uid); //작업지시서 번호
	$(opener.location).attr("href", "javascript:getWorkOrder();");
	//$(opener.location).attr("href", "javascript:getWorkOrderItem();");
	window.close();
}
//-->
</script>
			...
			<!-- PAGE CONTENT BEGINS -->
				...
				<div class="col-xs-5" style="float:right">
					<div class="input-group">						
						<input type="text" class="form-control search-query" name="search_txt" id="search_txt" placeholder="작업지시서 번호" onkeypress="if(event.keyCode==13) {search(); return false;}"/>
			...
			<table id="simple-table" class="table  table-bordered table-hover">
				<tr>
					...
					while($t = mysql_fetch_object($result)) {
						...
						$w = mysql_fetch_object($result22);
							//작업지시서 안의 품목갯수 가져오기
							$item_cnt = mysql_num_rows($result22);
							$item_cnt2 = $item_cnt -1;
						//echo  $w->process;
						$sql = "select uid from erp_item where item_cd='".addslashes($w->item_cd)."'";
						//echo  $sql."<BR>";
						$item = mysql_fetch_object(mysql_query($sql));
					?>
				<tr onclick="postWork('<?=$t->work_cd?>','<?=$w->item_cd?>','<?=$w->item_nm?>','<?=$w->standard1?>','<?=$w->material?>','<?=$w->unit?>','<?=$w->cnt?>','<?=$t->uid?>')" style="cursor:pointer">
					...
					<? }else{ ?>
						<td class="center"><?=$w->item_nm?>&nbsp;외&nbsp;<?=$item_cnt2?>&nbsp;개</th>
					<? }?>
