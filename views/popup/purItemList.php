...
<script>
function postItem(item_cd, item_nm, standard, pur_unit_price) {
	...
	if(idx >= 0) {
		...
		tag += "<td><input type='text' class='form-control unit_price' name='pur_unit_price[]' id='pur_unit_price_" + flag + "' value='" + pur_unit_price + "' onkeyup='calculation(" + flag + ")' /></td>";
...
?>
					<tr>
						...
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard?>','<?=number_format($t->pur_unit_price)?>')"><?=$t->item_cd?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard?>','<?=number_format($t->pur_unit_price)?>')"><?=$t->item_nm?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard?>','<?=number_format($t->pur_unit_price)?>')"><?=$t->standard?></span></td>
					</tr>			
<?
}				
?>
