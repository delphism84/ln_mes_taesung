...
$sql = "select b.* ,a.work_cd as work_cd, a.start_dt as start_dt from ".$table.$where." order by a.uid desc  limit ".($page-1)*$block.", ".$block;
//echo $sql; 
$result = query($sql);
...
function postWorkOrder(obj1,obj2,obj3,obj4,obj5,obj6,obj7) {
	...
	$("#order_qty",opener.document).val(obj7); 
	$(opener.location).attr("href", "javascript:getProductPerfReportsLotNo('"+obj6+"');");
		...
		<div class="col-xs-12">
			...
			<table id="simple-table" class="table  table-bordered table-hover">
				<tr>
					...
					?>
				<tr onclick="postWorkOrder('<?=$t->work_cd?>','<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$item->uid?>','<?=$t->order_cnt?>')" style="cursor:pointer">
