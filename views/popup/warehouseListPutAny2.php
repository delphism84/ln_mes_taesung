...
function postWarehouse(lnc,lnn){
	...
	var flag = $(opener.document).find("#warehouse_cdFlag").val();
	var arr = [];
	var warehouse_cd = [];

	$.each($(opener.document).find("#warehouse_cd") , function () {
		arr.push($(this).val());
	});
	...
	if(idx >= 0) {
		...
		var flag = flag;
		<?if ($cd=="" && $nm=="" ){?>
			$(opener.document).find("#warehouse_cd").val(lnc);
			$(opener.document).find("#warehouse_nm").val(lnn);
		<?}else{?>
			$(opener.document).find("#<?=$cd?>").val(lnc);
			$(opener.document).find("#<?=$nm?>").val(lnn);
		
		<?}?>
		//$(opener.document).find("#warehouse_cdFlag").val(Number(flag) + 1);
	}
		self.close();
}
...
?>
					<tr>
						<td><a href="javascript:void(0)" style="cursor:pointer" onclick="postWarehouse('<?=$t->warehouse_cd?>','<?=$t->warehouse_nm?>')"><?=$t->warehouse_cd?></a></td>
						<td><a href="javascript:void(0)" style="cursor:pointer" onclick="postWarehouse('<?=$t->warehouse_cd?>','<?=$t->warehouse_nm?>')"><?=$t->warehouse_nm?></a></td>
					</tr>			
<?
}				
?>
