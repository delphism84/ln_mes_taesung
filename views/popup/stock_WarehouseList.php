...
function postWarehouse(warehouse_cd, warehouse_nm) {
	$(opener.document).find("#warehouse_cd_"+<?=$flag?>).val(warehouse_cd);
	$(opener.document).find("#warehouse_nm_"+<?=$flag?>).val(warehouse_nm);
	self.close();
}	
</script>
		...
		<div class="col-xs-12">
			...
			<form name="frm" id="frm" method="post" action="index.php" enctype="multipart/form-data" />
				...
				<table id="simple-table" class="table  table-bordered table-hover">
					<tr>
						<th class="col-xs-6" style="background-color:#f1f1f1"><i class="ace-icon fa fa-caret-right blue"></i> 창고명</th>
					</tr>
<?
