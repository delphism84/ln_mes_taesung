...
if (  isset ( $gubun ) ){ 
	if( $item_gb != "all"){
		$where = " where (item_cd like '%".trim($search_txt)."%' or item_nm like '%".trim($search_txt)."%') and item_gb='".$item_gb."'";
	}else{
...
function postItem(item_cd, item_nm, standard1, material, unit, remain_cnt, remark) {
	...
	//} else {
		...
		tag += "<td><input type='text' class='form-control text-right cnt' name='cnt[]' id='cnt_" + flag + "' onkeyup='calculation(" + flag + ");input_comma(this);' onclick='this.select();' onblur='calculationTotal(" + flag + ");' /></td>";
		tag += "<td><input type='text' class='form-control text-right' name='remain_cnt[]' id='remain_cnt_" + flag + "' value='" + remain_cnt + "'  readonly/></td>";
		tag += "<td><input type='text' class='form-control text-right' name='box_cnt[]' id='box_cnt_" + flag + "' value='" + box_cnt + "'  /></td>";
		//tag += "<td><input type='text' class='form-control' name='remark[]' id='remark_" + flag + "' value='" + remark + "'  /></td>";
		tag += "<td><input type='text' class='form-control lot_no' name='lot_no_cd[]' id='lot_no_cd_" + flag + "' onclick=\"lotnocdFlag2(" + flag + ");lot_no_reg(" + flag + ",'"+ item_cd +"','"+ standard1 +"','"+ remain_cnt +"');\" value='' placeholder='[click]' readonly /><input type='hidden' name='lot_no_nm[]'  id='lot_no_nm_" + flag + "'/></td>";
		tag +="<td><input type='text' class='work_cd' name='work_cd[]'  id='work_cd_" + flag + "' onclick=\"lotnocdFlag2(" + flag + ");lot_no_reg(" + flag + ",'"+ item_cd +"','"+ standard1 +"','"+ remain_cnt +"');\" value='' placeholder='[click]' readonly/><input type='hidden' name='warehouse_nm[]'  id='warehouse_nm_" + flag + "' readonly/><input type='hidden' name='warehouse_cd[]'  id='warehouse_cd_" + flag + "' value='' readonly/></td>";
		tag += "</tr>";
...
function setPage(page){
	...
	location.href = "?page=" + page + "&search_txt=" + search_txt + "&item_gb=" + $("#item_gb option:selected").val() + "&gubun=y";
}
...
while($t = mysql_fetch_object($result)) {
	$sql2 = "select sum(remain_cnt) as remain_cnt from erp_stock_inout where item_cd='".$t->item_cd."' and standard1='".$t->standard1."'"; 
...
?>
					<tr>
						...
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=number_format($remain_cnt)?>','<?=$t->remark?>')"><?=$t->item_cd?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=number_format($remain_cnt)?>','<?=$t->remark?>')"><?=$t->item_nm?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=number_format($remain_cnt)?>','<?=$t->remark?>')"><?=$t->standard1?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=number_format($remain_cnt)?>','<?=$t->remark?>')"><?=$t->material?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=number_format($remain_cnt)?>','<?=$t->remark?>')"><?=$t->unit?></span></td>
					</tr>			
<?
}				
?>
