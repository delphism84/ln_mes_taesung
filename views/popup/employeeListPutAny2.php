...
function postEmployee(emp_id,emp_nm) {
	...
	<?if ($cd=="" && $nm=="" ){?>
	$(opener.document).find("#emp_id").val(emp_id);
	$(opener.document).find("#emp_nm").val(emp_nm);
	<?}else{?>
	$(opener.document).find("#<?=$cd?>").val(emp_id);
	$(opener.document).find("#<?=$nm?>").val(emp_nm);
	<?}?>
	self.close();
}
// 페이지 세트
...
?>
				<tr>
					...
					<td class='center'><a href="javascript:void(0)" style="cursor:pointer" onclick="postEmployee('<?=$t->emp_id?>','<?=$t->emp_nm?>')"><?=$t->emp_nm?></a></td>
				</tr>			
<?
}				
?>
