...
$sql = "select * from erp_standard1 where item_cd='".$item_cd."'";
...
function postStandard(standard) {
	$(opener.document).find("#standard1").val(standard);
	self.close();
}	
</script>
	...
	<div class="row">
		<div class="col-xs-12">
			<table id="simple-table" class="table  table-bordered table-hover">
				<tr>
					<th class="col-xs-4 center" style="background-color:#f1f1f1">품목코드</th>
...
?>
				<tr>
					<td class='center'><span style="cursor:pointer" onclick="postStandard('<?=$t->standard?>')"><?=$t->item_cd?></span></td>
...
?>
<?if ($t->item_cd==""){?>
<tr><td class='center' colspan='2'>등록된 품목코드가 없습니다.</tr>
<?}?>
			</table>			
		</div>
	</div><!-- /.row -->
