...
function postOrder(uid,order_cd, delivery_dt, account_cd, account_nm, manager) {
	...
	$(opener.document).find("#end_dt").val(delivery_dt);
	$(opener.document).find("#workplan_item").val(delivery_dt);
...
?>
					<tr>
						<td class='center'><span style="cursor:pointer" onclick="postOrder('<?=$t->uid?>','<?=$t->order_cd?>','<?=substr($t->delivery_dt,0,10)?>','<?=$t->account_cd?>','<?=$t->account_nm?>','<?=$t->manager?>')"><?=$t->order_cd?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postOrder('<?=$t->uid?>','<?=$t->order_cd?>','<?=substr($t->delivery_dt,0,10)?>','<?=$t->account_cd?>','<?=$t->account_nm?>','<?=$t->manager?>')"><?=$t->account_nm?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postOrder('<?=$t->uid?>','<?=$t->order_cd?>','<?=substr($t->delivery_dt,0,10)?>','<?=$t->account_cd?>','<?=$t->account_nm?>','<?=$t->manager?>')"><?=substr($t->create_dt,0,10)?></span></td>
					</tr>			
<?
}				
?>
