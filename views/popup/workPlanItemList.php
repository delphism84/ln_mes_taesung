...
function postItem(item_cd, item_nm, standard1, unit, cnt, remain_cnt, shortage_cnt, unit_price, supply_price, tax, total_price, account_cd, account_nm) {
	...
	*/
	//$(opener.document).find("#workplan_item_bom").empty();
	...
	if(idx >= 0) {
		...
		tag += "<td><input type='text' class='form-control id-btn-dialog1 account_cd' name='account_cd[]' id='account_cd_" + flag + "' value='" + account_cd + "' onclick='accountcdFlag(" + flag + ")' readonly /></td>";
		tag += "<td><input type='text' class='form-control account_nm' name='account_nm[]' id='account_nm_" + flag + "' value='" + account_nm + "'  /></td>";
		...
		$(opener.document).find("#workplan_item_bom tbody").append(tag);
...
?>
					<tr>
						...
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($ccnt->remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>')"><?=$t->item_cd?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($ccnt->remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>')"><?=$t->item_nm?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($ccnt->remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>')"><?=$t->standard1?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($ccnt->remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>')"><?=$t->material?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($ccnt->remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>')"><?=$t->unit?></span></td>
					</tr>			
<?
}				
?>
