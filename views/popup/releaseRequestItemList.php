...
function postItem(uid, fid, item_cd, item_nm, standard1, material, unit, cnt, remain_cnt, shortage_cnt, unit_price, supply_price, tax, total_price,account_cd, account_nm,lot_no_cd) {
		var flag = $(opener.document).find("#flag").val();
	<?if ($mode !="modify")	{?>
		if (flag=='1')
		{
			$(opener.document).find("#release_request_item tbody").empty(); //최초 tbody 초기화
		}
	<?}?>
	var tag = "";
	...
	*/
	...
	//} else {
		...
		tag += "<td><input type='text' class='form-control remain_cnt text-right' name='remain_cnt[]' id='remain_cnt_" + flag + "' onkeyup='calculation(" + flag + ");input_comma(this);' onclick='this.select();' onblur='calculationTotal(" + flag + ");' value='" + remain_cnt + "' readonly/></td>";
		...
		//tag += "<td><input type='text' class='form-control lot_no' name='lot_no_cd[]' id='lot_no_cd_" + flag + "' onclick='lotnocdFlag(" + flag + ");lot_no_reg(" + flag + ")' value='' readonly /><input type='hidden' name='lot_no_nm[]'  id='lot_no_nm_" + flag + "'/></td>";
		tag += "<td><input type='text' class='form-control lot_no' name='lot_no_cd[]' id='lot_no_cd_" + flag + "' onclick=\"lotnocdFlag(" + flag + ");lot_no_reg(" + flag + ",'"+ item_cd +"','"+ standard1 +"','"+ remain_cnt +"')\" value=''  placeholder='[click]' readonly /><input type='hidden' name='lot_no_nm[]'  id='lot_no_nm_" + flag + "'/></td>";
		tag += "<td><input type='text' class='form-control lot_no' name='warehouse_nm[]' id='warehouse_nm_" + flag + "' onclick=\"lotnocdFlag(" + flag + ");lot_no_reg(" + flag + ",'"+ item_cd +"','"+ standard1 +"','"+ remain_cnt +"')\" value='' placeholder='[click]' readonly/><input type='hidden' name='warehouse_cd[]'  id='warehouse_cd_" + flag + "'/></td>";
		tag += "</tr>";

		$(opener.document).find("#release_request_item tbody").append(tag);
	...
	<div class="row">
		...
		?>
					<tr>
						...
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->uid?>','<?=$t->fid?>','<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>','<?=$t->lot_no_cd?>')"><?=$t->item_cd?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->uid?>','<?=$t->fid?>','<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>','<?=$t->lot_no_cd?>')"><?=$t->item_nm?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->uid?>','<?=$t->fid?>','<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>','<?=$t->lot_no_cd?>')"><?=$t->standard1?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->uid?>','<?=$t->fid?>','<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>','<?=$t->lot_no_cd?>')"><?=$t->unit?></span></td>
					</tr>			
<?
}				
?>
