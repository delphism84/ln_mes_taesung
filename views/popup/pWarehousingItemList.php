...
function postItem(uid, fid, item_cd, item_nm, standard1, unit, cnt, remain_cnt, shortage_cnt, unit_price, supply_price, tax, total_price,account_cd, account_nm) {
	...
	*/
	...
	if(idx >= 0) {
		...
		//tag += "<td><input type='text' class='form-control shortage_cnt text-right' name='shortage_cnt[]' id='shortage_cnt_" + flag + "' onkeyup='calculation(" + flag + ");input_comma(this);' onclick='this.select();' onblur='calculationTotal(" + flag + ");' value='" + shortage_cnt + "' readonly/></td>";
		...
		tag += "<td><input type='text' class='form-control text-right supply_price' name='supply_price[]' id='supply_price_" + flag + "' onclick='this.select();' onkeyup='input_comma(this);' onblur='calculationTotal(" + flag + ");' value='" + supply_price + "'  /></td>";
		...
		tag += "<td><input type='text' class='form-control lot_no' name='lot_no_cd[]' id='lot_no_cd_" + flag + "' onclick='lotnocdFlag(" + flag + ");lot_no_reg(" + flag + ")' placeholder='[click]' readonly /><input type='hidden' name='lot_no_nm[]'  id='lot_no_nm_" + flag + "' /></td>";
		tag += "<td><input type='hidden' name='inspection_cd[]' id='inspection_cd_" + flag + "' readonly /><button class='btn btn-info btn-xs' id='btn_" + flag + "' type='button' onclick=\"inspectionPop('" + flag + "', '" + uid + "', '" + fid + "', '" + item_cd + "','" + item_nm + "','" + standard1 + "','" + unit + "')\"><i class='ace-icon fa fa-check bigger-110'></i>등록</button></td>";
		tag += "</tr>";

		$(opener.document).find("#warehousing_item tbody").append(tag);
...
?>
					<tr>
						...
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->uid?>','<?=$t->fid?>','<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($stock->remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>')"><?=$t->item_cd?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->uid?>','<?=$t->fid?>','<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($stock->remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>')"><?=$t->item_nm?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->uid?>','<?=$t->fid?>','<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($stock->remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>')"><?=$t->standard1?></span></td>
						<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->uid?>','<?=$t->fid?>','<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$cnt?>','<?=number_format($stock->remain_cnt)?>','<?=number_format($bcnt)?>','<?=number_format($t->unit_price)?>','<?=number_format($supply_price)?>','<?=number_format($tax)?>','<?=number_format($total_price)?>','<?=$t->account_cd?>','<?=$t->account_nm?>')"><?=$t->unit?></span></td>
					</tr>			
<?
}				
?>
