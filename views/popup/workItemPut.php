...
function postItem(item_cd, item_nm, standard, material, unit, remain_cnt, shortage_cnt, order_cnt) {
	...
	if(idx >= 0) {
		...
		tag += "<td><input type='text' class='form-control onlynum' name='order_cnt[]' id='order_cnt" + flag + "' value='" + order_cnt + "'  /></td>";
		//tag += "<td class='center'><select name='warehouse_cd[]' id='warehouse_cd_" + flag + "' class='warehouse'>" + warehouse + "</select></td>";
		tag += "<td><input type='text' class='form-control warehouse_nm' name='warehouse_nm[]' id='warehouse_nm_" + flag + "' value='' onclick='warehouse_cdFlag(" + flag + ");warehouse_cd_reg(" + flag + ")' readonly  /><input type='hidden' name='warehouse_cd[]' id='warehouse_cd_" + flag + "' value='' placeholder='생산 공장' readonly/></td>";
		tag += "<td><input type='text' name='machine_nm[]' id='machine_nm_" + flag + "' placeholder='생산 설비명' value='' onclick=\"centerOpenWindow('views/popup/machineListPut2.php?flag="+ flag +"', '생산기기 리스트', 600, 600)\" readonly/><input type='hidden' name='machine_cd[]' id='machine_cd_" + flag + "' value='' readonly/></td>";
		tag += "</tr>";
...
while($t = mysql_fetch_object($result)) {
	if($t->item_gb == "component") $item_gb = "자재";
		...
		$sql = "select remain_cnt from erp_stock where item_cd='".$t->item_cd."' and standard='".$t->standard1."'";
		$stock = @mysql_fetch_object(mysql_query($sql2));
			$cnt = 1;
			$total_cnt = $cnt;
			// 부족재고수량
			if($stock->remain_cnt=="") $remain_cnt="0";
			$bcnt = $total_cnt - $stock->remain_cnt;

		$sql = "select cnt from erp_bom where item_cd='".$t->item_cd."' and standard='".$t->standard1."'";
		$bom = @mysql_fetch_object(mysql_query($sql2));
		if($bom->cnt=="") $cnt="0";

?>
				<tr>
					<td class='center'><?=$item_gb?></td>
					<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=$remain_cnt?>','<?=$bcnt?>','<?=$cnt?>')"><?=$t->item_cd?></span></td>
					<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=$remain_cnt?>','<?=$bcnt?>','<?=$cnt?>')"><?=$t->item_nm?></span></td>
					<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=$remain_cnt?>','<?=$bcnt?>','<?=$cnt?>')"><?=$t->standard1?></span></td>
					<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=$remain_cnt?>','<?=$bcnt?>','<?=$cnt?>')"><?=$t->material?></span></td>
					<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$t->unit?>','<?=$remain_cnt?>','<?=$bcnt?>','<?=$cnt?>')"><?=$t->unit?></span></td>
				</tr>			
<?
}				
?>
