...
$sql = "select * ,a.work_cd as awork_cd from ".$table.$where." order by a.uid desc  limit ".($page-1)*$block.", ".$block;
//echo $sql; 
$result = query($sql);
...
function setPage(page){
	...
	location.href = "?page=" + page + "&search_txt=" + search_txt + "&state=" + $("#state option:selected").val();
}
		...
		<div class="col-xs-12">
			...
			<table id="simple-table" class="table  table-bordered table-hover">
				<tr>
					...
					?>
				<tr onclick="postWork('<?=$t->awork_cd?>','<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->material?>','<?=$item->uid?>')" style="cursor:pointer">
					<td class="center"><?=$t->awork_cd?></th>
