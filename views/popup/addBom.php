...
function postItem(item_cd, item_nm, standard1, unit, material) {
	...
	if(idx >= 0) {
		...
		tag += "<td class='col-xs-2'><input type='text' class='form-control item_cd' name='item_cd[]' id='item_cd_" + flag + "'  placeholder='품목선택을 하시려면 클릭하세요' value='" + item_cd + "' readonly /></td>";
		tag += "<td class='col-xs-2'><input type='text' class='form-control' name='item_nm[]' id='item_nm_" + flag + "' value='" + item_nm + "' readonly /></td>";
		tag += "<td class='col-xs-2'><input type='text' class='form-control standard1' name='standard1[]' id='standard1_" + flag + "' value='" + standard1 + "' readonly /></td>";
		tag += "<td class='col-xs-1'><input type='text' class='form-control cnt' name='material[]' id='material_" + flag + "' value='" + material + "' readonly /></td>";
		tag += "<td class='col-xs-1'><input type='text' class='form-control cnt' name='unit[]' id='unit_" + flag + "' value='" + unit + "' readonly /></td>";
		tag += "<td class='col-xs-1'><input type='text' class='form-control cnt' name='cnt[]' id='cnt_" + flag + "' value='0'  /></td>";
		//tag += "<td class='col-xs-2'><input type='text' class='form-control cnt' name='lot_no[]' id='lot_no_" + flag + "' value='' /></td>";
		tag += "</tr>";

		$(opener.document).find("#bom tbody").append(tag);
...
function setPage(page){
	...
	//location.href = "?page=" + page + "&where=" + $("#where").val() + "&item_gb=" + $("#item_gb option:selected").val();
}
...
?>
				<tr>
					...
					<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$t->material?>')"><?=$t->item_cd?></span></td>
					<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$t->material?>')"><?=$t->item_nm?></span></td>
					<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$t->material?>')"><?=$t->standard1?></span></td>
					<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$t->material?>')"><?=$t->material?></span></td>
					<td class='center'><span style="cursor:pointer" onclick="postItem('<?=$t->item_cd?>','<?=$t->item_nm?>','<?=$t->standard1?>','<?=$t->unit?>','<?=$t->material?>')"><?=$t->unit?></span></td>
				</tr>			
<?
}				
?>
