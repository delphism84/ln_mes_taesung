		...
		<div class="page-content">
			...
			<div class="row">
				<div class="col-xs-12">
					<!-- PAGE CONTENT BEGINS -->
					<!-- 테이블 -->
					<table id="document_list" class="table  table-bordered table-hover">
						<thead>
							<tr>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">NO</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">문서그룹</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">제목</th>
								<th class="col-xs-2 center" style="background-color:#f1f1f1">첨부파일</th>
...
function get_document(page){
	...
	$.getJSON("ajax/document.php",{"page":page, "mode":"get_document", "where":search, "account_type":account_type},
		function(json){
			if(json != null) {
				...
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "<td><a href='" + json[i].upload_file + "'>" + json[i].upload_file + "</a></td>";
					tag += "<td>" + json[i].reg_date + "</td>";
					tag += "</tr>";
				}
			} 

			$("#document_list tbody").html(tag);
...
function delete_select(){
	if(confirm("선택하신 거래처 정보를 삭제하시겠습니까?")) {
		...
		var dataString = "mode=delete_data&table=data&uids=" + $("#chk_uids").val();
		$.ajax({
			type : "post",
			url : "ajax/ajax.php",
			data : dataString,
