						...
						<table id="fixed_assets_code_list" class="table  table-bordered table-hover">
							<thead>
								<tr>
									...
									<th class="col-xs-2 center" style="background-color:#f1f1f1">자산코드</th>
									<th class="col-xs-2 center" style="background-color:#f1f1f1">자산명</th>
									<th class="col-xs-2 center" style="background-color:#f1f1f1">자산계정명</th>
									<th class="col-xs-1 center" style="background-color:#f1f1f1">상각방법</th>
									<th class="col-xs-1 center" style="background-color:#f1f1f1">내용연수단위</th>
									<th class="col-xs-1 center" style="background-color:#f1f1f1">내용연수</th>
									<th class="col-xs-2 center" style="background-color:#f1f1f1">잔존가치</th>
									<th class="col-xs-1 center" style="background-color:#f1f1f1">상태</th>
								</tr>
							</thead>
							<tbody></tbody>
						</table>
			...
			<div class="clearfix form-actions center">
				<div class="col-md-12">
					...
					<div class="col-xs-6 right" style="text-align:right">
						...
						<button class="btn btn-info" type="button" onclick="fixedAssetsCode_reg('0','0')">
        ...
        <div class="modal-content" >
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">고정자산코드</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="fixedAssetsCode_reg_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
...
<div id="id-btn-dialog2" class="modal fade" >
    <div class="modal-dialog modal-lg" style="width:70%">
        <div class="modal-content" >
            ...
            <div class="modal-body">
                <p><iframe src="" id="fixedAssetsCode_modify_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
// 거래처 리스트 가져오기
function getFixedAssetsCode(page){
	...
	$.getJSON("ajax/fixedasset.php",{"page":page, "mode":"getFixedAssetsCode", "rpp" : rpp, "adjacents" : adjacents, "where":search, "account_gb":account_gb, "search_choice" : search_choice, "txt" : txt, "table" : table},
		function(json){
			if(json != null) {
				...
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "</label>";
					tag += "</td>";
					tag += "<td class='center' ><a href='#' onclick='fixedAssetsCode_modiy(" + json[i].uid + "," + json[i].fac_cd + ")' >" + json[i].fac_cd + "</a></td>";
					tag += "<td class='center'>" + json[i].fac_nm + "</td>";
					tag += "<td class='center'>" + json[i].asset_ac_nm + "</td>";
					tag += "<td class='center'>" + json[i].depreciable_type + "</td>";
					tag += "<td class='center'>" + json[i].service_life_unit + "</td>";
					tag += "<td class='center'>" + json[i].service_life + "</td>";
					tag += "<td class='center'>" + commaSplit(json[i].salvage_value) + "</td>";
					tag += "<td class='center'>" + json[i].status + "</td>";

					tag += "</tr>";
				}
			}else{
				tag += "<tr>";
					tag += "<td colspan=10 class='center'>등록된 자산등록코드 데이터가 없습니다.";
					tag += "</td>";
				tag += "</tr>";
			}
...
function deleteSelect(){
	if(confirm("선택하신 전표 정보를 삭제하시겠습니까? 다른 데이터와 연동된 전표 정보는 삭제되지 않습니다.")) {
		...
		var dataString = "mode=deleteSelectstatement&table=erp_g_statement&uids=" + $("#check_uids").val();
		$.ajax({
			type : "post",
...
<script type="text/javascript">
<!--
	function fixedAssetsCode_reg(cidx, acd)
	{
	...
	var url = "index.php?controller=accounting&action=registFixedAssetsCodePop&idx="+cidx+"&pop=Y&fac_cd="+acd+"&dialouid=id-btn-dialog2";
	//var url = "./views/accounting/card/cardcompany_remark_list_pop.php?&uid="+cidx+"&pop=Y&ddd="+cidx;
	$("#fixedAssetsCode_reg_frame").attr("src", url);
	}

	function fixedAssetsCode_modiy(uid, dt)
	{
	...
	var url = "index.php?controller=accounting&action=modifyFixedAssetsCodePop&uid="+uid+"&pop=Y&fac_cd="+dt+"&dialouid=id-btn-dialog2";
	$("#fixedAssetsCode_modify_frame").attr("src", url);
	}

	function close_popup()
	{	
		$.modal.close();
		$("#fixedAssetsCode_reg_frame").attr("src", "about:blank");
	}
