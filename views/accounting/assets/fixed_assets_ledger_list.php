...
<!-- PAGE CONTENT BEGINS ---------------------------------------------------------------------------------------------------------------------------------------->
					...
					<div style="margin-top:10px">
						<table id="fixed_assets_ledger_list" class="table  table-bordered table-hover">
							<thead>
								<tr>
									...
									<th style="background-color:#f1f1f1">계정명</th>
									<th class="col-xs-1 center" style="background-color:#f1f1f1">자산코드</th>
									<th class="col-xs-2 center" style="background-color:#f1f1f1">자산명</th>
									<th class="col-xs-1 center" style="background-color:#f1f1f1">구입일자</th>
									<th class="col-xs-1 center" style="background-color:#f1f1f1">수량</th>
									<th class="col-xs-1 center" style="background-color:#f1f1f1">취득금액</th>
									<th class="col-xs-1 center" style="background-color:#f1f1f1">감가상각충당금</th>
									<th class="col-xs-1 center" style="background-color:#f1f1f1">적요</th>
									<th class="col-xs-1 center" style="background-color:#f1f1f1">상태</th>
									<th class="col-xs-1 center" style="background-color:#f1f1f1">상세내역</th>
								</tr>
							</thead>
							<tbody></tbody>
						</table>
			...
			<div class="clearfix form-actions center">
				<div class="col-md-12">
					...
					<div class="col-xs-6 right" style="text-align:right">
						...
						<button class="btn btn-info" type="button" onclick="generalStatement_reg('0','0')">
        ...
        <div class="modal-content" >
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">일반전표</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="generalStatement_reg_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
...
<div id="id-btn-dialog2" class="modal fade" >
    <div class="modal-dialog modal-lg" style="width:70%">
        <div class="modal-content" >
            ...
            <div class="modal-body">
                <p><iframe src="" id="generalStatement_modify_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
// 거래처 리스트 가져오기
function getGeneralStatement(page){
	...
	$.getJSON("ajax/statement.php",{"page":page, "mode":"getGeneralStatement", "rpp" : rpp, "adjacents" : adjacents, "where":search, "account_gb":account_gb, "search_choice" : search_choice, "txt" : txt, "table" : table},
		function(json){
			if(json != null) {
				...
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "<input type='checkbox' class='ace flat chk' value='" + json[i].gid + "' />";
					tag += "<span class='lbl'></span>";
					tag += "</label>";
					tag += "</td>";
					tag += "<td class='center' ><a href='javascript:void(0);'  onclick='generalStatement_modiy(" + json[i].gid + "," + json[i].statement_dt + ", " + json[i].statement_ca + ")' >" + json[i].statement_dt + "-" + json[i].statement_ca + "</a></td>";
					tag += "<td class='center'>" + json[i].invoiceType + "</td>";
					tag += "<td class='center'>" + commaSplit(json[i].total_price) + "</td>";
					tag += "<td class='center'>" + json[i].account_nm + "</td>";
					tag += "<td class='center'>" + json[i].etc + "</td>";
					tag += "<td class='center'><a href='javascript:void(0);'  onclick='javascript:openWinPrint(" + json[i].gid + ");'>인쇄</a></td>";
					tag += "<td class='center'>" + json[i].etc + "</td>";
					tag += "<td class='center'>" + json[i].etc + "</td>";
					tag += "<td class='center'>" + json[i].etc + "</td>";
					tag += "<td class='center'>" + json[i].etc + "</td>";
			...
			$("#fixed_assets_ledger_list tbody").html(tag);
...
function deleteSelect(){
	if(confirm("선택하신 전표 정보를 삭제하시겠습니까? 다른 데이터와 연동된 전표 정보는 삭제되지 않습니다.")) {
		...
		var dataString = "mode=deleteSelectstatement&table=erp_g_statement&gids=" + $("#check_gids").val();
		$.ajax({
			type : "post",
...
<script type="text/javascript">
<!--
	function generalStatement_reg(cidx, acd)
	{
	...
	var url = "index.php?controller=accounting&action=registGeneralStatementPop&idx="+cidx+"&pop=Y&aci_cd="+acd;
	//var url = "./views/accounting/card/cardcompany_remark_list_pop.php?&gid="+cidx+"&pop=Y&ddd="+cidx;
	$("#generalStatement_reg_frame").attr("src", url);
	}

	function generalStatement_modiy(gid, dt, ca)
	{
	...
	var url = "index.php?controller=accounting&action=modifyGeneralStatementPop&gid="+gid+"&pop=Y&statement_dt="+dt+"&statement_ca="+ca;
	//var url = "./views/accounting/card/cardcompany_remark_list_pop.php?&gid="+cidx+"&pop=Y&ddd="+cidx;
	...
	{	
		$.modal.close();
		$("#generalStatement_reg_frame").attr("src", "about:blank");
	}
//-->
</script>