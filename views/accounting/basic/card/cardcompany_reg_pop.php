		...
		<div class="breadcrumbs ace-save-state" id="breadcrumbs">
				...
				<div class="col-xs-12">
					...
					<form id="ac_frm" method="post" action="index.php">
						...
						<input type="hidden" name="action" id="action" value="registCardCompanyInsert" />
						<!-- 테이블 -->
						<table id="simple-table" class="table table-bordered table-hover">
							<tr>
								<th class="col-xs-2" style="background-color:#f1f1f1" colspan=2><i class="fa fa-arrow-circle-right" aria-hidden="true"></i> 카드사코드</th>
							</tr>
							<tr>
								<th class="col-xs-2" style="background-color:#f1f1f1" > 카드사 코드</th>
								<td class="col-xs-10">
									<input type="text" name="card_ccode" id="card_ccode" /> <input type="button" id="confirmCardCnum" value="중복확인" class="btn btn-inverse btn-sm"> <span id="id_signed"></span>
								</td>
							</tr>
							<tr>
								<th class="col-xs-2" style="background-color:#f1f1f1"> 카드사명</th>
								<td class="col-xs-10"><input type="text" name="card_cname" id="card_cname" /></td>
							</tr>
							<tr>
								<th class="col-xs-2" style="background-color:#f1f1f1"> 들어올계좌</th>
								<td class="col-xs-10"><input type="text" class="form-control" name="card_cnum" id="card_cnum" /></td>
								</tr>
							<tr>
								<th class="col-xs-2" style="background-color:#f1f1f1"> 계정명</th>
								<td class="col-xs-10">
							...
							<tr>
								<th class="col-xs-2" style="background-color:#f1f1f1"> 수수료율</th>
								<td class="col-xs-10"><input type="text" class="form-control" name="com_rate" id="com_rate" /></td>
								</tr>
							<tr>
...
<!--
	function formSubmit(){
	if(!check_str($("#card_cname").val(),"카드사명")) return false;
	if(!check_str($("#card_ccode").val(),"카드사코드")) return false;
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#confirmCardCnum').click(function() {
			var card_ccode = $('#card_ccode').val();
			if(card_ccode == ''){
				alert('카드사 코드를 입력하세요!');
				$('#card_ccode').focus();
				return;
			...
			$.getJSON('ajax/cardcompany.php',{"mode":"confirmCardCompany", "card_ccode":card_ccode},function(json){
				if(json == 'success'){
						count = 0;
						$('#id_signed').html('이미 존재하는 코드 입니다.').css('color','red');
						$('#card_ccode').val('').focus();
				}else {
					count = 1;
		...
		//input태그에 key이벤트 연결
		$('#ac_frm #card_ccode').keyup(function(){
			count = 0;
			$('#id_signed').html('');
		});
		/*
