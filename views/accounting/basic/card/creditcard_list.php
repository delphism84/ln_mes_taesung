		...
		<div class="page-content">
			...
			<div class="row">
				<div class="col-xs-12">
					<!-- PAGE CONTENT BEGINS -->
					<!-- 테이블 -->
					<table id="creditcard_list" class="table  table-bordered table-hover">
						<thead>
							<tr>
								<th class="text-center" style="width:20px">
									<span class="form-checkbox"><input name="chkAll" class="" type="checkbox" ><label class="" name="chkAll"></label></span>
								</th>
								<th class="col-xs-2 center" style="background-color:#f1f1f1">카드번호</th>
								<th class="col-xs-2 center" style="background-color:#f1f1f1">구분</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">카드명</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">결제계좌명</th>
								<th class="col-xs-2 center" style="background-color:#f1f1f1">계정명</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">카드관리자</th>
			...
			<div class="clearfix form-actions center">
				<div class="col-md-12">
					...
					<div class="col-xs-6">
						<button class="btn btn-info" type="button" id="btn-footer-Change" onclick="creditcard_reg()">
...
*/
	...
	$.getJSON("ajax/creditcard.php",{"page":page, "mode":"get_creditcard", "where":search, "account_type":account_type},
		function(json){
			if(json != null) {
				...
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "</td>";
					tag += "<td class='left'><a href='javascript:void(0);'  onclick='creditcard_mod(" + json[i].idx + "," + json[i].card_num + ");'>" + json[i].card_num + "</a></td>";
					tag += "<td class='center'><a href='javascript:void(0);'  onclick='creditcard_reg(" + json[i].idx + "," + json[i].card_num + ");'>" + json[i].card_type + "</a></td>";
					tag += "<td class='center'>" + json[i].card_name + "</td>";
					tag += "<td class='center'>" + json[i].pay_account_name + "</td>";
					tag += "<td class='center'>" + json[i].aci_nm + "</td>";
					tag += "<td class='center'>" + json[i].card_manager + "</td>";
			...
			$("#creditcard_list tbody").html(tag);
...
function delete_select(){
	if(confirm("선택하신 거래처 정보를 삭제하시겠습니까?")) {
		...
		var dataString = "mode=delete_data&table=erp_creditcard&uids=" + $("#chk_uids").val();
		$.ajax({
			type : "post",
			url : "ajax/ajax.php",
			data : dataString,
...
<div id="id-btn-dialog1" class="modal fade" >
    <div class="modal-dialog" style="width:800px">
        <div class="modal-content" >
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">신용카드등록</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="creditcardReg_frame" frameborder="0" width="99%" height="500" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
<div id="id-btn-dialog2" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">신용카드수정</h4>
            </div>
            <div class="modal-body">
                <p><iframe src="" id="creditcardMod_frame" frameborder="0" width="99%" height="500" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
	...
	function remark_view(cidx, acd)
{
	...
	//var url = "./views/accounting/card/creditcard_remark_list_pop.php?&uid="+cidx+"&pop=Y&ddd="+cidx;
...
<script type="text/javascript">
<!--
	function creditcard_reg(cidx, acd)
{
	...
	var url = "index.php?controller=accounting&action=registCreditCardPop&idx="+cidx+"&pop=Y&aci_cd="+acd;
	//var url = "./views/accounting/card/creditcard_remark_list_pop.php?&uid="+cidx+"&pop=Y&ddd="+cidx;
	$("#creditcardReg_frame").attr("src", url);
}
function close_popup()
{	
	$.modal.close();
	$("#creditcardReg_frame").attr("src", "about:blank");
}
//-->
</script>
...
<script type="text/javascript">
<!--
	function creditcard_mod(cidx, acd)
{
	...
	//var url = "./views/accounting/card/creditcard_remark_list_pop.php?&uid="+cidx+"&pop=Y&ddd="+cidx;
	$("#creditcardMod_frame").attr("src", url);
}
function close_popup()
{	
	$.modal.close();
	$("#creditcardMod_frame").attr("src", "about:blank");
}
//-->
</script>