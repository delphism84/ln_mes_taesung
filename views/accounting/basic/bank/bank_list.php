		...
		<div class="page-content">
			...
			<div class="row">
				<div class="col-xs-12">
					<!-- PAGE CONTENT BEGINS -->
					<!-- 테이블 -->
					<table id="bank_list" class="table  table-bordered table-hover">
						<thead>
							<tr> 
								<th class="text-center" style="width:20px">
									<span class="form-checkbox"><input name="chkAll" class="" type="checkbox" ><label class="" name="chkAll"></label></span>
								</th>
								<th class="col-xs-2 center" style="background-color:#f1f1f1">계좌번호</th>
								<th class="col-xs-2 center" style="background-color:#f1f1f1">계좌명</th>
								<th class="col-xs-2 center" style="background-color:#f1f1f1">계정명(계정코드) </th>
								<th class="col-xs-2 center" style="background-color:#f1f1f1">검색창내용 </th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">적요</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">외화통장</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">사용</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">이체정보</th>
							</tr>
						</thead>
						<tbody></tbody>
					</table>
			...
			<div class="clearfix form-actions center">
				<div class="col-md-12">
					...
					<div class="col-xs-6">
						<button class="btn btn-info" type="button" id="btn-footer-Change" onclick="bank_reg()">
...
*/
	...
	$.getJSON("ajax/bank.php",{"page":page, "mode":"get_bank_list", "where":search, "account_type":account_type},
		function(json){
			if(json != null) {
				...
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "</td>";
					tag += "<td class='left'><a href='javascript:void(0);'  onclick='bank_mod(" + json[i].idx + "," + json[i].bank_num + ");'>" + json[i].bank_num + "</a></td>";
					tag += "<td class='center'><a href='javascript:void(0);'  onclick='bank_reg(" + json[i].idx + "," + json[i].bank_num + ");'>" + json[i].bank_name + "</a></td>";
...
function delete_select(){
	if(confirm("선택하신 거래처 정보를 삭제하시겠습니까?")) {
		...
		var dataString = "mode=delete_data&table=erp_bank_list&uids=" + $("#chk_uids").val();
		$.ajax({
			type : "post",
			url : "ajax/ajax.php",
			data : dataString,
...
<div id="id-btn-dialog1" class="modal fade" >
    <div class="modal-dialog" style="width:800px">
        <div class="modal-content" >
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">통장계좌등록</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="bankReg_frame" frameborder="0" width="99%" height="500" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
<div id="id-btn-dialog2" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">통장계좌수정</h4>
            </div>
            <div class="modal-body">
                <p><iframe src="" id="bankMod_frame" frameborder="0" width="99%" height="500" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
	...
	function remark_view(cidx, acd)
{
	...
	//var url = "./views/accounting/card/bank_remark_list_pop.php?&uid="+cidx+"&pop=Y&ddd="+cidx;
	...
	function bank_reg(cidx, acd)
{
	...
	var url = "index.php?controller=accounting&action=registBankPop&idx="+cidx+"&pop=Y&aci_cd="+acd;
	$("#bankReg_frame").attr("src", url);
}
function close_popup()
{	
	$.modal.close();
	$("#bankReg_frame").attr("src", "about:blank");
}
//-->
</script>
	...
	function bank_mod(cidx, acd)
{
	...
	var url = "index.php?controller=accounting&action=modifyBankPop&idx="+cidx+"&pop=Y&aci_cd="+acd;
	$("#bankMod_frame").attr("src", url);
}
function close_popup()
{	
	$.modal.close();
	$("#bankMod_frame").attr("src", "about:blank");
}
//-->
</script>