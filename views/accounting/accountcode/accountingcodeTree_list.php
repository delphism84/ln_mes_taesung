		...
		<div class="page-content">
					...
					<table id="account_code_list" class="table  table-bordered table-hover">
						<thead>
							<tr>
								...
								<th class="col-xs-1 center" style="background-color:#f1f1f1">계정</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">대차구분</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">계정종류</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">평가구분</th>
								<th class="col-xs-1 center" style="background-color:#f1f1f1">사용구분</th>
			...
			<div class="clearfix form-actions center">
				<div class="col-md-12">
					...
					<div class="col-xs-6">
						<button class="btn btn-info" type="button" id="btn-footer-Change">
						...
						<button class="btn btn-info" type="button" id="btn-footer-Excel">
							<i class="ace-icon fa fa-check"></i>
							Excel
						</button>
					</div>
...
function get_account_code(page){
	var rpp = 50;
	var adjacents = 4;
	var tag = "";
...
*/
	...
	$.getJSON("ajax/account_code.php",{"page":page, "mode":"get_account_code", "where":search, "account_type":account_type, "rpp" : rpp, "adjacents" : adjacents},
		function(json){
			if(json != null) {
				...
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "</td>";
					//tag += "<td class='left'><a href='index.php?controller=accounting&action=modifyAccountingCodePop&uid=" + json[i].uid + "'>" + json[i].aci_nm + "</a></td>";
					tag += "<td class='left'><a href='javascript:void(0);'  onclick='account_code_mod(" + json[i].uid + "," + json[i].aci_cd + ");'>" + json[i].aci_nm + "</a></td>";
					
					if (json[i].high_account_code !="5")
					{
						//tag += "<td class='center'><a href='index.php?controller=accounting&action=registAccountingCodePop&uid=" + json[i].uid + "' target='blur()'>추가</a></td>";
						tag += "<td class='center'><a href='javascript:void(0);'  onclick='account_code_reg(" + json[i].uid + "," + json[i].aci_cd + ");'>추가</a></td>";
					}else{
						tag += "<td></td>";
					}
					...
					tag += "<td class='center'><a href='javascript:void(0);'  onclick='remark_view(" + json[i].uid + "," + json[i].aci_cd + ");'>등록</a></td>";
					tag += "</tr>";
				}
			} 
...
<div id="id-btn-dialog1" class="modal fade" >
    <div class="modal-dialog" style="width:800px">
        <div class="modal-content" >
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">계정코드등록</h4>

            </div>
            <div class="modal-body">
                <p><iframe src="" id="accountCodeReg_frame" frameborder="0" width="99%" height="600" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
<div id="id-btn-dialog2" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header" id="modal-header1">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                 <h4 class="modal-title">계정코드수정</h4>
            </div>
            <div class="modal-body">
                <p><iframe src="" id="accountCodeMod_frame" frameborder="0" width="99%" height="300" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
<script type="text/javascript">
<!--
	function account_code_reg(cidx, acd)
{
	...
	var url = "index.php?controller=accounting&action=registAccountingCodePop&idx="+cidx+"&pop=Y&aci_cd="+acd;
	//var url = "./views/accounting/accountcode/account_code_remark_list_pop.php?&uid="+cidx+"&pop=Y&ddd="+cidx;
	$("#accountCodeReg_frame").attr("src", url);
}
function close_popup()
{	
	$.modal.close();
	$("#accountCodeReg_frame").attr("src", "about:blank");
}
//-->
</script>
...
<script type="text/javascript">
<!--
	function account_code_mod(cidx, acd)
{
	...
	//var url = "./views/accounting/accountcode/account_code_remark_list_pop.php?&uid="+cidx+"&pop=Y&ddd="+cidx;
	$("#accountCodeMod_frame").attr("src", url);
}
function close_popup()
{	
	$.modal.close();
	$("#accountCodeMod_frame").attr("src", "about:blank");
}
//-->
</script>