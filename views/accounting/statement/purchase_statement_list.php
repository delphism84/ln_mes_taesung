...
<!-- PAGE CONTENT BEGINS ---------------------------------------------------------------------------------------------------------------------------------------->
					...
					<div style="margin-top:10px">
						<table id="purchase_statement_list" class="table  table-bordered table-hover">
							<thead>
								<tr>
		...
		function(json){
			if(json != null) {
				...
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "<td class='center'> 매입전표 </td>";
					tag += "<td class='center'>" + commaSplit(json[i].supply_price) + "</td>";
					tag += "<td class='center'>" + json[i].account_nm + "</td>";
					tag += "<td class='center'>" + json[i].remark + "</td>";
					tag += "<td class='center'><a href='javascript:void(0);'  onclick='javascript:openWinPrint(" + json[i].pid + ");'>인쇄</a></td>";
			...
			$("#purchase_statement_list tbody").html(tag);
...
<!--
	...
	var url = "index.php?controller=accounting&action=registPurchaseStatementPop&idx="+cidx+"&pop=Y&aci_cd="+acd+"&dialogid=id-btn-dialog1";
	...
	var url = "index.php?controller=accounting&action=modifyPurchaseStatementPop&pid="+pid+"&pop=Y&statement_dt="+dt+"&statement_ca="+ca+"&dialogid=id-btn-dialog2";
	//var url = "./views/accounting/card/cardcompany_remark_list_pop.php?&pid="+cidx+"&pop=Y&ddd="+cidx;
