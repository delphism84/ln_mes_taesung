			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form name="frm" id="frm" method="post" action="index.php">
						...
						<input type="hidden" name="action" id="action" value="registSalesStatementUpdate" />
							...
							<tr>
								...
								<td class="col-xs-11" colspan=3>
									<select name="invoiceType" id="invoiceType">
										<option value="1" <? if($t->invoiceType == "1") echo "selected"; ?>>종이(세금)계산서</option>
										<option value="2" <? if($t->invoiceType == "2") echo "selected"; ?>>전자(세금)계산서-신규</option>
										<option value="3" <? if($t->invoiceType == "3") echo "selected"; ?>>기재사항착오정정</option>
										<option value="4" <? if($t->invoiceType == "4") echo "selected"; ?>>공급가액변동</option>
										<option value="5" <? if($t->invoiceType == "5") echo "selected"; ?>>환입</option>
										<option value="6" <? if($t->invoiceType == "6") echo "selected"; ?>>계약의해제</option>
										<option value="7" <? if($t->invoiceType == "7") echo "selected"; ?>>내국신용장개설</option>
										<option value="8" <? if($t->invoiceType == "8") echo "selected"; ?>>착오에의한이중발행</option>
									</select>
								</td>
							</tr>
							<tr>
						...
						<table id="statement" class="table  table-bordered table-hover" style="margin-top:10px">
							...
							<tbody>
								...
								<tr>
									...
									<th class="center col-xs-2" style="background-color:#f1f1f1"><SPAN></SPAN></th>
									<th class="center col-xs-2" style="background-color:#f1f1f1"><SPAN class="supply_price_Total"><?=number_format($t->total_price)?></SPAN></th>
									<th class="center col-xs-2" style="background-color:#f1f1f1"><SPAN class="tax_Total"><?=number_format($t->total_tax)?></SPAN></th>
									<th class="center col-xs-6" style="background-color:#f1f1f1"><SPAN class="remarkAmount "><?=number_format($t->total_price + $t->total_tax)?></SPAN></th>
								</tr>
							</tbody>
						</table>
						...
						<table id="simple-table" class="table  table-bordered table-hover">
							<tr>
								...
								<td class="col-xs-10">
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input type="text" name="receiptfee_cd" id="receiptfee_cd" value="<?=$t->receiptfee_cd?>" readonly />
												<input type="text" name="receiptfee_nm" id="receiptfee_nm" value="<?=$t->receiptfee_nm?>" readonly />
												<span class="input-group-addon btn-purple" id="id-btn-dialog8">
													<i class="ace-icon fa fa-search icon-on-right bigger-110" style="color:#ffffff"></i>
												</span>
												&nbsp;<input type="text" name="receiptfee_price" id="receiptfee_price" value="<?=$t->receiptfee_price?>" class="text-right" />
											</div>
										</span>
									</div>
								</td>
									...
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												...
												&nbsp;<input type="text" name="receiptbank_num" id="receiptbank_num" value="<?=$t->receiptbank_num?>" class="text-right" />
											</div>
										</span>
									</div>
								</td>
							...
							<tr>
								...
								<td class="col-xs-5">
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input type="text" name="accountsrecev_cd" id="accountsrecev_cd" value="<?=$t->accountsrecev_cd?>" readonly />
												<input type="text" name="accountsrecev_nm" id="accountsrecev_nm" value="<?=$t->accountsrecev_nm?>" readonly />
												<span class="input-group-addon btn-purple" id="id-btn-dialog100">
													<i class="ace-icon fa fa-search icon-on-right bigger-110" style="color:#ffffff"></i>
												</span>
												&nbsp;<input type="text" name="accounts_recev_price" id="accounts_recev_price" value="<?=number_format($t->accounts_recev_price)?>" class="text-right" readonly />
											</div>
										</span>
									</div>
								</td>
							...
							<tr>
								...
								<button class="btn btn-link btn-xs" type="button" onclick="tax_invoice_reg('<?=$t->statement_dt."-".$t->statement_ca?>','s','<?=$t->accounts_recev_price?>')">
...
function getSalesStatementItem(){
	var sid = $("#sid").val();
	var currentFlag = $("#flag").val();
	var tag = "";

	$.getJSON("ajax/statement.php",{"mode":"getSalesStatementItem", "sid" : sid},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					if(i < 4) {
						...
						$("#supply_price_" + currentFlag).val(json[i].supply_price);
						$("#tax_" + currentFlag).val(json[i].tax);
						...
						tag += "<td><input type='text' class='form-control aci_nm' name='aci_nm[]' id='aci_nm_" + currentFlag + "' readonly /></td>";
						tag += "<td><input type='text' class='form-control text-right' name='supply_price[]' id='supply_price_" + currentFlag + "' onkeyup='calculation(" + currentFlag + ")'/></td>";
						tag += "<td><input type='text' class='form-control text-right' name='tax[]' id='tax_" + currentFlag + "' onkeyup='calculation(" + currentFlag + ")' /></td>";
						tag += "<td><input type='text' class='form-control' name='remark[]' id='remark_" + currentFlag + "'  /></td>";
						tag += "<td><input type='text' class='form-control' name='writer[]' id='writer_" + currentFlag + "'  /></td>";
						tag += "</tr>";
						alert(tag)
...
//	var values = Number(removeComma(cnt)) * Number(removeComma(supply_price));
	...
	if(tax_type == 1 || tax_type == 4  || tax_type == 6  || tax_type == 8  || tax_type == 12  || tax_type == 13) {
		...
		//$("#unit_price_" + flag).val(commaSplit(Math.round(values + tax)));
	} else {
		var tax = 0;
		//$("#unit_price_" + flag).val(commaSplit(Math.round(values + tax)));
	}
...
jQuery(function($) {
	...
	$( "#id-btn-dialog10" ).on('click', function(e) {
		...
		var dialog = $( "#dialog-message10" ).removeClass('hide').dialog({
			width : 500,
			height:500,
			modal: true,
			title: "<div class='widget-header widget-header-small'><h4 class='smaller'><i class='ace-icon fa fa-minus-square'></i>계좌 리스트</h4></div>",
