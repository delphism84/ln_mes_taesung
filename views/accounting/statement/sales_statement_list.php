			...
			<div class="clearfix form-actions center">
				<div class="col-md-12">
					...
					<div class="col-xs-6 right" style="text-align:right">
						...
						<button class="btn btn-info" type="button" onclick="salesStatement_reg('0','0')">
    ...
    <div class="modal-dialog modal-lg" style="width:70%">
        <div class="modal-content" >
            ...
            <div class="modal-body">
                <p><iframe src="" id="salesStatement_reg_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
...
<div id="id-btn-dialog2" class="modal fade" >
    <div class="modal-dialog modal-lg" style="width:70%">
        <div class="modal-content" >
            ...
            <div class="modal-body">
                <p><iframe src="" id="salesStatement_modify_frame" frameborder="0" width="99%" height="670" marginwidth="0" marginheight="0" scrolling="yes"></iframe></p>
            </div>
            <!-- <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary">Save changes</button>
...
// 거래처 리스트 가져오기
function getSalesStatement(page){
	var table = "erp_s_statement";
	var rpp = 15;
	var adjacents = 4;
	var tag = "";
	...
	$.getJSON("ajax/statement.php",{"page":page, "mode":"getSalesStatement", "rpp" : rpp, "adjacents" : adjacents, "where":search, "account_gb":account_gb, "search_choice" : search_choice, "txt" : txt, "table" : table},
		function(json){
			if(json != null) {
				...
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "<input type='checkbox' class='ace flat chk' value='" + json[i].sid + "' />";
					tag += "<span class='lbl'></span>";
					tag += "</label>";
					tag += "</td>";
					tag += "<td class='center' ><a href='javascript:void(0);'  onclick='salesStatement_modiy(" + json[i].sid + "," + json[i].statement_dt + ", " + json[i].statement_ca + ")' >" + json[i].statement_dt + "-" + json[i].statement_ca + "</a></td>";
					...
					tag += "<td class='center'>" + json[i].summary + "</td>";
					tag += "<td class='center'><a href='javascript:void(0);'  onclick='javascript:openWinPrint(" + json[i].sid + ");'>인쇄</a></td>";

					tag += "</tr>";
				}
			}else{
				tag += "<tr>";
					tag += "<td colspan=10 class='center'>등록된 매출전표 리스트가 없습니다.";
					tag += "</td>";
				tag += "</tr>";
			}
...
<script type="text/javascript">
<!--
	function salesStatement_reg(cidx, acd)
	{
	...
	var url = "index.php?controller=accounting&action=registSalesStatementPop&idx="+cidx+"&pop=Y&aci_cd="+acd+"&dialogid=id-btn-dialog1";
	//var url = "./views/accounting/card/cardcompany_remark_list_pop.php?&sid="+cidx+"&pop=Y&ddd="+cidx;
	$("#salesStatement_reg_frame").attr("src", url);
	}

	function salesStatement_modiy(sid, dt, ca)
	{
	...
	var url = "index.php?controller=accounting&action=modifySalesStatementPop&sid="+sid+"&pop=Y&statement_dt="+dt+"&statement_ca="+ca+"&dialogid=id-btn-dialog2";
	//var url = "./views/accounting/card/cardcompany_remark_list_pop.php?&sid="+cidx+"&pop=Y&ddd="+cidx;
	$("#salesStatement_modify_frame").attr("src", url);
	}

	function close_popup()
	{	
		$.modal.close();
		$("#salesStatement_reg_frame").attr("src", "about:blank");
	}
//-->
</script>
