...
// 파일이 있는 디렉토리
	...
	if (!empty($mem_sn)) {
		if ($downtype=="order"){ //주문서
			$downfiledir = $_SERVER["DOCUMENT_ROOT"]."/userfiles/order/". $mem_sn."/";
		}else if ($downtype=="salesDaily"){
			$downfiledir = $_SERVER["DOCUMENT_ROOT"]."/userfiles/daily/". $mem_sn."/";
		}else if ($downtype=="estimate"){ 
			$downfiledir = $_SERVER["DOCUMENT_ROOT"]."/userfiles/estimate/". $mem_sn."/";
		}else if ($downtype=="approval"){ 
			$downfiledir = $_SERVER["DOCUMENT_ROOT"]."/userfiles/approval/". $mem_sn."/";
		}else{
			$downfiledir = $_SERVER["DOCUMENT_ROOT"]."/userfiles/estimate/". $mem_sn."/";
		}
	} else {
		if ($downtype=="order"){ //주문서
			$downfiledir = $_SERVER["DOCUMENT_ROOT"]."/userfiles/order/". $memberCsn."/";
		}else if ($downtype == "salesDaily"){
			$downfiledir = $_SERVER["DOCUMENT_ROOT"]."/userfiles/daily/". $memberCsn."/";
		}else if ($downtype == "estimate"){ 
			$downfiledir = $_SERVER["DOCUMENT_ROOT"]."/userfiles/estimate/". $memberCsn."/";
		}else if ($downtype == "approval"){ 
			$downfiledir = $_SERVER["DOCUMENT_ROOT"]."/userfiles/approval/". $memberCsn."/";
		}else{
			$downfiledir = $_SERVER["DOCUMENT_ROOT"]."/userfiles/estimate/". $memberCsn."/";
		}
	}
