						...
						<form name="frm" id="frm" method="post" action="index.php">
							...
							<table id="simple-table" class="table table-bordered table-bordered">
								<tr>
									...
									<td class="col-xs-10">
										<input type="button" class="btn btn-xs btn-danger" value="데이터백업 하기" onclick="backup()" />
								...
								<tr>
									...
									<td class="col-xs-10">
										<input type="button" class="btn btn-xs btn-success" value="데이터복원 하기" onclick="restore()" />
...
<script>
function backup() {
	var dataString = "mode=backup";
	$.ajax({
		type : "post",
		data : dataString,
		url : "ajax/config.php",
		success : function(str) {
			if(str == "success") alert("데이터를 백업하였습니다");
			else alert(str);
		}
	});
}
...
function restore() {
	if(confirm("현재 데이터를 삭제하고 저장된 백업데이터로 복원하시겠습니까?")) {
		var dataString = "mode=restore";
		$.ajax({
			type : "post",
			data : dataString,
			url : "ajax/config.php",
			success : function(str) {
				if(str == "success") alert("데이터를 복원하였습니다");
				else alert(str);
			}
		});
	}
