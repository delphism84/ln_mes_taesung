			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form name="frm" id="frm" method="post" action="index.php" enctype="multipart/form-data">
						...
						<input type="hidden" name="action" id="action" value="updateCustomer" />
						<input type="hidden" name="uid" id="uid" value="<?=$t->uid?>" />
						<!-- 테이블 -->
						<table id="simple-table" class="table  table-bordered table-hover">
							<tr>
								<th class="col-xs-1" style="background-color:#f1f1f1">고객명</th>
								<td class="col-xs-5"><input type="text" name="customer_nm" id="customer_nm" value="<?=$t->customer_nm?>" /></td>
								<th class="col-xs-1" style="background-color:#f1f1f1">고객그룹</th>
								<td class="col-xs-5">
									<select name="customer_gb" id="customer_gb">
										<option value="일반" <? if($t->customer_gb == "일반") echo "selected"; ?>>일반</option>
										<option value="VIP" <? if($t->customer_gb == "VIP") echo "selected"; ?>>VIP</option>
									</select>
							...
							<tr>
								...
								<td class="col-xs-5">
									<div class="input-group">
										...
										<input class="form-control input-mask-mobile" type="text" name="customer_phone" id="customer_phone" value="<?=$t->customer_phone?>"/>
									</div>
								</td>
								<th class="col-xs-1" style="background-color:#f1f1f1">이메일</th>
								<td class="col-xs-5">
									<input type="text" name="customer_email" id="customer_email" value="<?=$t->customer_email?>"/>
								</td>
							</tr>
							<tr>
								...
								<td class="col-xs-11" colspan="3">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											<input class=" date-picker" name="customer_birthday" id="customer_birthday" type="text" data-date-format="yyyy-mm-dd" value="<?=substr($t->customer_birthday,0,10)?>" />
											<span class="input-group-addon">
							...
							<tr>
								...
								<td class="col-xs-11" colspan="3">
									<div class="input-group">
										<input type="text" class="form-control search-query" placeholder="우편번호" name="customer_zipcode" id="customer_zipcode" value="<?=$t->customer_zipcode?>" readonly />
									...
									<div style="margin-top:5px">
										<input type="text" class="form-control search-query" placeholder="주소" name="customer_address" id="customer_address" value="<?=$t->customer_address?>" />
									</div>
								</td>
							</tr>
						</table>
					...
					<form name="counsel_frm" id="counsel_frm" method="post" action="index.php" enctype="multipart/form-data">
						<input type="hidden" name="controller" id="controller" value="groupware" />
						<input type="hidden" name="action" id="action" value="registCounsel" />
						<input type="hidden" name="fid" id="fid" value="<?=$t->uid?>" />
						<!-- 테이블 -->
						<table id="simple-table" class="table  table-bordered table-hover">
							<tr>
								<th class="col-xs-1" style="background-color:#f1f1f1">상담구분</th>
								<td class="col-xs-5">
									<select name="counsel_gb" id="counsel_gb">
										<option value="제품문의">제품문의</option>
										<option value="제품하자">>제품하자</option>
									</select>

								</td>
								<th class="col-xs-1" style="background-color:#f1f1f1">상담일시</th>
								<td class="col-xs-5">
									<span class="input-icon input-icon-right">
										<div class="input-group">
											<input class=" date-picker" name="counsel_dt" id="counsel_dt" type="text" data-date-format="yyyy-mm-dd" value="<?=date("Y-m-d")?>" />
											<span class="input-group-addon">
												<i class="fa fa-calendar bigger-110"></i>
											</span>
										</div>
							...
							<tr>
								...
								<td class="col-xs-11" colspan="3">
									<textarea class="form-control" rows="5" name="counsel" id="counsel"></textarea>
								</td>
							</tr>
						</table>
					</form>
					...
					<div class="clearfix form-actions center">
						<div class="col-md-12">
							<button class="btn btn-info" type="button" onclick="counselFormSubmit()">
								<i class="ace-icon fa fa-check bigger-110"></i>
								상담등록
							</button>
							&nbsp; &nbsp; &nbsp;
					...
					<table id="counsel_list" class="table  table-bordered table-hover">
						<thead>
							<tr>
								<th class="col-xs-1" style="background-color:#f1f1f1">상담구분</th>
								<th class="col-xs-1" style="background-color:#f1f1f1">상담일</th>
								<th class="col-xs-1" style="background-color:#f1f1f1">상담내용</th>
								<th class="col-xs-1" style="background-color:#f1f1f1">상담사</th>
							</tr>
						</thead>
						<tbody></tbody>
					</table>
...
function getCounsel() {
	var tag = "";
	var fid = $("#fid").val();
	$.getJSON("ajax/groupware.php",{"mode":"getCounsel","fid":fid},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					tag += "<tr>";
					tag += "<td>" + json[i].counsel_gb + "</td>";
					tag += "<td>" + json[i].counsel_dt + "</td>";
					tag += "<td>" + json[i].counsel + "</td>";
					tag += "<td>" + json[i].emp_nm + "</td>";
					tag += "</tr>";
				}
			}

			$("#counsel_list tbody").html(tag);
		}
	);
}
