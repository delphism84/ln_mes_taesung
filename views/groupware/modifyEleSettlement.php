	...
	<div class="main-content-inner">
		...
		<div class="page-content">
					...
					<form name="frm" id="frm" method="post" action="index.php" enctype="multipart/form-data" />
						...
						<table id="simple-table" class="table  table-bordered table-hover">
							<tr>
								...
								<td class="col-xs-5"><input type="text" class="form-control" name="title" id="title" value="<?=$t->title?>" /></td>
								...
								<td colspan="3" class="col-xs-11">
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input type="text" name="purchase_cd" id="purchase_cd" value="<?=$t->purchase_cd?>" />
												<span class="input-group-addon btn-purple" onclick="openWin('<?=$t->purchase_cd?>')">
							...
							<tr>
								<td colspan="4"><textarea name="comment" id="comment"><?=$t->comment?></textarea></td>
			...
			<div class="clearfix form-actions center">
				<div class="col-md-12">
					<button class="btn btn-info" type="button" onclick="approval()">
...
function openWin(purchase_cd){
	var win = window.open("index.php?controller=purchase&action=viewPagePurchaseDemand&purchase_cd=" + purchase_cd,"구매요청서","width=950,height=800, status=no, toolbar=no");
}
...
function approval() {
	var dataString = "uid=" + $("#uid").val() + "&mode=approval";
	$.ajax({
		type : "post",
		...
		success : function(str) {
			if(str == "success") {
				alert("결재를 하였습니다");
				location.href = "index.php?controller=groupware&action=listPageEleSettlement";
			}
		}
	});
}
