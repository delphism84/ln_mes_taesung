...
switch($mode) {
	...
	case "get_creditcard_remark" :   //적요 리스트
		$where = " where 1=1";
		/*
		if($account_gb == "all") {
			$where .= "";
		...
		*/
		...
		$query = "select * from erp_creditcard_remark".$where;
		...
		$rpp = 1000;  # record/page 
		$adjacents = $adjacents; # 양 옆에 표시될 페이지 수  
		$query = "select count(*) from erp_creditcard_remark".$where;
		$query = mysql_query($query); 
		list($total) = mysql_fetch_row($query); 
		$query = "select * from erp_creditcard_remark".$where." order by idx  limit ".($page-1)*$rpp.", ".$rpp;
	...
	case "deleteSelectAccount" :
		...
		for($i = 0 ; $i <= sizeof($array_uid) ; $i++) {
			$query = "delete from erp_creditcard where uid=".$array_uid[$i];
			mysql_query($query);
		}
	...
	case "confirmAccountCode" :
		$query = "select count(*) as cnt from erp_creditcard where aci_cd=".$aci_cd;
		//echo $query;
		$query = mysql_query($query); 
