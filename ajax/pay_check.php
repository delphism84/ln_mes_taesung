...
switch($mode) {
	...
	case "getPayCheck" :
		...
		*/
		...
		$query = "select * from erp_pay_check".$where;
		...
		$query = "select * from erp_pay_check".$where." order by uid  limit ".($page-1)*$rpp.", ".$rpp;
		//echo $query;
		...
		while($t = @mysql_fetch_object($result)) {
			...
			*/
			$re[$i]['scdate'] = $t->scdate;
			$re[$i]['ecdate'] = $t->ecdate;
			$re[$i]['paydate'] = $t->paydate;
			$re[$i]['lastday'] = $t->lastday;
			$re[$i]['bonusday'] = $t->bonusday;
			$re[$i]['sbonusdate'] = $t->sbonusdate;
			$re[$i]['ebonusdate'] = $t->ebonusdate;
			$re[$i]['payrateflag'] = $t->payrateflag;
			/*
			...
			*/
			...
			$re[$i]['use_yn'] = $use_yn;
			$re[$i]['payrate'] = $t->payrate;
			$re[$i]['bonusapplyflag'] = $t->bonusapplyflag;
			$re[$i]['adjustyy'] = $t->adjustyy;
			$re[$i]['paydes']= $t->paydes;
			$re[$i]['paycomment']= $t->paycomment;
			$re[$i]['totalpayamount']= $t->totalpayamount;
