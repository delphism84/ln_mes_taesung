...
switch($mode) {
	...
	case "getSalesStatement" :
		...
		while($t = @mysql_fetch_object($result)) {
			...
			$re[$i]['in_type_ectax_flag'] = $t->in_type_ectax_flag;
			...
			$in_type_ectax_flag_text = GetInTypeEctaxFlag($t->in_type_ectax_flag);
			$re[$i]['in_type_ectax_flag'] = $in_type_ectax_flag_text;
