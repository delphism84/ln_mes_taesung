...
switch($mode) {
	...
	case "getCard" :
		...
		*/
		...
		$query = "select * from erp_creditcard".$where;
		...
		$query = "select count(*) from erp_creditcard".$where;
		$query = mysql_query($query); 
		list($total) = mysql_fetch_row($query); 
		$query = "select * from erp_creditcard".$where." order by idx limit ".($page-1)*$rpp.", ".$rpp;
		...
		while($t = @mysql_fetch_object($result)) {
			...
			$re[$i]['no'] = $no;
			$re[$i]['idx'] = $t->uid;
			$re[$i]['card_num'] = $t->card_num;
			$re[$i]['card_name'] = $t->card_name;
			$re[$i]['card_type'] = $t->card_type;
			$re[$i]['pay_account_name'] = $t->pay_account_name;
			$re[$i]['card_manager'] = $t->card_manager;
