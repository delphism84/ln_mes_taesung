		...
		<div class="page-content">
			<div>				
				...
				<div class="input-group">
					...
					<input type="text" name="account_nm" id="account_nm" style="height:35px; margin-left:3px" onclick="showModal('accountModal')" placeholder="[거래처검색Click]" readonly />
					<input type="button" class="btn btn-xs btn-purple" onclick="search()" value="검색" style="height:35px; margin-bottom:4px" />			
				</div>
				<div class="col-xs-12">
					<?	
						echo "<input type='button' class='comm_title' value='업체별 세부 매입현황' / >";
						echo "</div>";
						$this->noCheckTable("tb","거래일자,품목,규격,거래처,수량,단가");
						$this->paging();
					?>
				</div>
...
function getData(page){
	var tag = "";
	var parameter = {"mode" : "getAccountPurchaseList", "account_cd" : $("#account_cd").val(), "s_year" : $("#s_year").val(), "s_month" : $("#s_month").val()};
		...
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					tag += "<td style='vertical-align:middle'>" + json[i].days + " 일" + "</td>";
