		...
		<div class="page-content">
			<div>				
				<div class="col-xs-12">
					<div class="col-xs-12">
						<?
						echo "<input type='button' class='comm_title' value='업체별 판매순위표' />";
						echo "</div>";
						$this->noCheckTable("tb","순위,매출업체,매출액,비중");
						$this->paging();
						?>
					</div>
				</div>
...
function getData(page){
	...
	var tag = "";
	var all ="";
	var parameter = {"mode" : "getAccountSaleRanking", "account_cd" : $("#account_cd").val(), "start_dt" : $("#start_dt").val(), "end_dt" : $("#end_dt").val()};
		...
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					//tag += "<td style='vertical-align:middle; text-align:right'>" + comma(json[i].ave) + "</td>";
			...
			$("#tb tbody").html(tag);
			$("#tb tbody").append("<td colspan='9' style='vertical-align:middle; text-align:center; height: 50px; border:1px solid #ccc;'>매출총액 : "+all+"</td>");
		}
	);
}
