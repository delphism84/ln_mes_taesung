					...
					<div class="col-xs-12" style="border:1px solid #ccc; height:100%; padding-top:10px;">
						...
						<div>
							<div class="input-group">								
								<input type="text" name="search_txt" id="search_txt" style="float:right; height:35px; width:100%;"/>													
						...
						if($_SESSION['login_level'] >= 99){
							//$this->table("tb","품번,품명,년도,1월,2월,3월,4월,5월,6월,7월,8월,9월,10월,11월,12월,총생산량,시간당생산량");
						} else {
							//$this->noCheckTable("tb","품번,품명,년도,1월,2월,3월,4월,5월,6월,7월,8월,9월,10월,11월,12월,총생산량,시간당생산량");
						}
	...
	<div class="modal-dialog" style="width:1400px;">
		<div class="modal-content">
			...
			<div class="modal-body" style="height:500px">
				<form id="frm">
					<input type="hidden" name="mode" id="mode" value="registProductionStatus" />
					...
					<table class="table table-bordered">
						...
						<tr>
							<? $this->th("해당년도") ?>
							<td colspan="3"><input type="text" name="year" id="year" value="2018" /></td>
						</tr>
						<tr>
							<? $this->th("1월") ?>
							<td><input type="text" class="comma onlynum" name="jan" id="jan" /></td>						
							<? $this->th("2월") ?>
							<td><input type="text" class="comma onlynum" name="feb" id="feb" /></td>
						</tr>
						<tr>
							<? $this->th("3월") ?>
							<td><input type="text" class="comma onlynum" name="mar" id="mar" /></td>						
							<? $this->th("4월") ?>
							<td><input type="text" class="comma onlynum" name="apr" id="apr" /></td>
						</tr>
						<tr>
							<? $this->th("5월") ?>
							<td><input type="text" class="comma onlynum" name="may" id="may" /></td>						
							<? $this->th("6월") ?>
							<td><input type="text" class="comma onlynum" name="jun" id="jun" /></td>
						</tr>
						<tr>
							<? $this->th("7월") ?>
							<td><input type="text" class="comma onlynum" name="jul" id="jul" /></td>						
							<? $this->th("8월") ?>
							<td><input type="text" class="comma onlynum" name="aug" id="aug" /></td>
						</tr>
						<tr>
							<? $this->th("9월") ?>
							<td><input type="text" class="comma onlynum" name="sep" id="sep" /></td>						
							<? $this->th("10월") ?>
							<td><input type="text" class="comma onlynum" name="oct" id="oct" /></td>
						</tr>
						<tr>
							<? $this->th("11월") ?>
							<td><input type="text" class="comma onlynum" name="nov" id="nov" /></td>						
							<? $this->th("12월") ?>
							<td><input type="text" class="comma onlynum" name="dec" id="dec" /></td>
						</tr>
					</table>
				</form>
			</div>
...
function selectDelete() {
	deleteSelect("production_status");
...
function getData(page){
	var tag = "";
	var parameter = {"mode" : "getProductionStatusList", "rpp" : $("#per").val(), "adjacents" : 4, "page" : page, "where" : $("#where").val()};
		...
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					...
					var total = Number(json[i].jan) + Number(json[i].feb) + Number(json[i].mar) + Number(json[i].apr) + Number(json[i].may) + Number(json[i].jun) + Number(json[i].jul) + Number(json[i].aug) + Number(json[i].sep) + Number(json[i].oct) + Number(json[i].nov) + Number(json[i].dec);
					
					var a = Number(total) / 4;
					var b = Number(a) / 22;
					var time_avg = Number(b) / 8;

					tag += "<td style='text-align:right'>" + comma(Math.floor(total)) + "</td>";
					tag += "<td style='text-align:right; font-weight:bold; color:red'>" + comma(Math.floor(time_avg)) + "</td>";
					tag += "</tr>";
				}
			} else {
