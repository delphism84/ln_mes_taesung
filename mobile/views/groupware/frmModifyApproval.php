				...
				<div class="col-xs-12" style="padding-top:10px">
					...
					<form name="frm" id="frm">						
						...
						<div style="text-align:right; margin-bottom:3px">
							<select name="document" id="document" onchange="changeTemplate(this.value)">
								<option value="none">양식을 선택하세요</option>
<?
$sql = "select * from approval_document";
$this->query($sql);
while($t = $this->fetch()) {
	echo "<option value='".$t->uid."'>".$t->title."</option>";
}
?>
							</select>
						</div>
						<table id="simple-table" class="table  table-bordered table-hover">
							<tr>
								...
								<td class="col-xs-5"><input type="text" name="approval_cd" id="approval_cd" value="<?=$app->approval_cd?>" validation="yes" err="문서번호를 입력하세요" /></td>
								<th class="col-xs-1" style="background-color:#f1f1f1">기안제목</th>
								<td class="col-xs-5"><input type="text" class="form-control" name="title" id="title" value="<?=$app->title?>" validation="yes" err="기안제목을 입력하세요" /></td>
							</tr>
							<tr>
								...
								<td class="col-xs-5">
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input type="text" name="approval_nm" id="approval_nm" value="<?=$line->line_nm?>" />
												<input type="hidden" name="approval_uid" id="approval_uid" value="<?=$app->approval_line?>" validation="yes" err="결재자를 지정하세요" />
												<span class="input-group-addon btn-danger" onclick="viewApprovalLineModal()" style="cursor:pointer">
													<i class="ace-icon fa fa-search icon-on-right bigger-110" style="color:#ffffff">결재라인</i>
												</span>
												<!-- <span class="input-group-addon btn-primary" onclick="location.href='index.php?controller=groupware&action=frmApprovalLine'" style="cursor:pointer">
								...
								<td class="col-xs-5">
									<div class="input-group">
										<span class="input-icon input-icon-right">
											<div class="input-group">
												<input type="text" name="refer_emp_nm" id="refer_emp_nm" onclick="showModal('employeeModal')" />
												<input type="hidden" name="refer_emp_id" id="refer_emp_id" />
							...
							<tr>
								<th class="col-xs-1" style="background-color:#f1f1f1; vertical-align:middle">
									<span class="input-group-addon btn-purple" style="cursor:pointer" onclick="viewEstimateModal()">
										<i class="ace-icon fa fa-search icon-on-right bigger-110" style="color:#ffffff">견적서</i>
									</span>
								</th>
								<td>
									<div>
										<input type="hidden" name="estimate_cd" id="estimate_cd" value="<?=$app->estimate_cd?>" />
										<textarea class="form-control" id="estimate_txt" name="estimate_txt"><?=$app->estimate_txt?></textarea>
									</div>
								</td>
								<th class="col-xs-1" style="background-color:#f1f1f1; vertical-align:middle">
									<span class="input-group-addon btn-purple" style="cursor:pointer" onclick="viewPurchaseModal()">
										<i class="ace-icon fa fa-search icon-on-right bigger-110" style="color:#ffffff"> 구매요청서</i>
									</span>
								</th>
								<td>
									<div>
										<input type="hidden" name="purchase_cd" id="purchase_cd" value="<?=$app->purchase_cd?>" />												
										<textarea class="form-control" id="purchase_txt" name="purchase_txt"><?=$app->purchase_txt?></textarea>
									</div>
								</td>
							</tr>
							<tr>
								<th class="col-xs-1" style="background-color:#f1f1f1; vertical-align:middle">
									<span class="input-group-addon btn-purple" style="cursor:pointer" onclick="viewShipmentModal()">
										<i class="ace-icon fa fa-search icon-on-right bigger-110" style="color:#ffffff">출하지시서</i>
									</span>
								</th>
								<td>
									<div>
										<input type="hidden" name="shipment_cd" id="shipment_cd" value="<?=$app->shipment_cd?>" />
										<textarea class="form-control" id="shipment_txt" name="shipment_txt"><?=$app->shipment_txt?></textarea>
									</div>
								</td>
								<th class="col-xs-1" style="background-color:#f1f1f1; vertical-align:middle">
									<span class="input-group-addon btn-purple" style="cursor:pointer" onclick="viewSpendingModal()">
										<i class="ace-icon fa fa-search icon-on-right bigger-110" style="color:#ffffff">지출결의서</i>
									</span>
								</th>
								<td>
									<div>
										<input type="hidden" name="spending_cd" id="spending_cd" value="<?=$app->spending_cd?>" />
										<textarea class="form-control" id="spending_txt" name="spending_txt"><?=$app->spending_txt?></textarea>
									</div>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<td colspan="4"><textarea name="comment" id="comment"><?=$app->comment?></textarea></td>
