				...
				<div class="input-group">
					<div class="col-xs-12">
						...
						if($_SESSION['login_level'] >= 99) {
							...
							//echo "<input type='button' class='btn btn-xs btn-pink' value='외주리스트' style='height:35px; margin-right:5px' />";									
		...
		<div class="page-content">	
			<div>
				<div class="col-xs-12">
					<div>
						<form id="order">						
							<input type='button' class='comm_title' value='외주리스트'/style="float:left;">
					...
					<!--
					<div>
						<div><input type="button" class="btn btn-xs btn-pink" value="발주품목" /></div>
						<? $this->noCheckTable("item_tb","품번=>2,품목명=>2,발주수량=>2,입고희망일=>2,상태=>1"); ?>
					</div>
					-->
				</div>
				<!--
										...
										<tr>
											<td align="center" valign="top" class=border_body>
												...
												<table id="simple-table" class="table  table-bordered table-hover">
													...
													<tr bgcolor="efefef">
														<td colspan="4"><strong>발주사 정보</strong></td>
													</tr>
													<tr>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-header" style="background:#007bff">
				<span style="font-weight:bold; color:#fff; font-size:14pt">발주품목</span>
			...
			<div class="modal-body" style="height:70%; overflow:scroll; overflow-x:hidden">
				<div>					
					<? $this->noCheckTable("item_tb","품번=>2,품목명=>4,발주수량=>2,상태=>2"); ?>
				</div>
			</div>
...
function refresh() {
	...
	$("#search_txt").val("");
	$("#page").val(1);
	$("#where").val("where status='외주발주'");
	getData(1);
}

function setClassify(classify){
	$("#where").val("where status='외주발주' and state='" + classify + "'");
	getData(1);
}
	...
	$.getJSON("ajax.php", {"parameter" : parameter}, function(json) {
		if(json != null) {
			for(var i = 0 ; i < json.length ; i++) {
				...
				tag1 += "<td style='text-align:right'>" + comma(json[i].cnt) + "</td>";
				...
				tag2 += "<td style='text-align:right'>" + comma(json[i].cnt) + "</td>";
