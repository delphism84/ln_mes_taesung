...
<div class="main-content">
	<div class="main-content-inner">
		...
		<div class="page-content">				
			<div>
				<div class="col-xs-12">
					<div><input type="button" class="comm_title" value="외주업체" /></div>
					<? $this->noCheckTable("tb","외주 업체명"); ?>
				</div>
...
<!--창고 품목 MODAL-->
<div class="modal fade" id="viewOutsourcingWarehouseItemModal" data-backdrop="static" data-keyboard="false">
				...
				<div>						
					<div class="col-xs-12">
						<div style="padding-top:10px">
							<!--<div><input type="button" class="btn btn-xs btn-pink" value="품목 리스트" /></div>-->
							<? $this->noCheckTable("warehouse_tb","품목코드=>3,품목명=>3,재고수량=>3,입고일자"); ?>
							<? $this->paging() ?>
						</div>
					</div>
				</div>
...
function getWarehouse(){
	var tag = "";
	var parameter = {"mode" : "getOutsourcingWarehouseList"};
	$.getJSON("ajax.php",{"parameter" : parameter},
		function(json){
			if(json != null) {
				...
				tag = "<tr><td colspan='1' style='padding:20px; color:red; font-weight:bold; text-align:center'>데이터가 존재하지 않습니다</td></tr>";
			}
...
function getData(){
	var tag = "";
	var parameter = {"mode" : "getOutsourcingWarehouseItem", "uid" : $("#uid").val()};
	$.getJSON("ajax.php",{"parameter" : parameter},
		function(json){
			if(json != null) {
				for(var i = 0 ; i < json.length ; i++){
					tag += "<tr onclick=\"item_toggle(this);\" style='cursor:pointer'>";
					tag += "<td>" + json[i].item_cd + "</td>";
					tag += "<td>" + json[i].item_nm + "</td>";
					//tag += "<td>" + json[i].standard + "</td>";
					//tag += "<td>" + json[i].unit + "</td>";
					tag += "<td>" + json[i].cnt + "</td>";
					tag += "<td><input type='button' class='btn btn-xs btn-primary' value='입고내역보기' /></td>";
					//tag += "<td><input type='button' class='btn btn-xs btn-danger' value='재고조정' /></td>";
					tag += "</tr>";
				}
			} else {
	...
	showModal('viewOutsourcingWarehouseItemModal');
}
