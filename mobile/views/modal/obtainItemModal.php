				...
				<div style="margin-bottom:5px">
					<div style="float:right">
						<div class="input-group">
							<select name="modal_item_search_classify" id="modal_item_search_classify" style="height:34px">
								<option value="0">= 검색구분 =</option>
								<option value="item_cd">품번</option>
								<option value="item_nm">품명</option>
							</select>
							<input type="text" class="search-query" placeholder="검색어" name="modal_item_search_txt" id="modal_item_search_txt" style="width:725px; height:34px; margin-left:5px"/>
	...
	$.getJSON("ajax.php", {"parameter" : parameter}, function(json) {
		if(json != null) {
			for(var i = 0 ; i < json.length ; i++){
				tag += "<tr onclick=\"postItem('" + json[i].item_cd + "', '" + json[i].item_nm + "', '" + json[i].standard + "', '" + json[i].unit + "')\" style='cursor:pointer'>";
