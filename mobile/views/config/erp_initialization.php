...
<?
$sql = "SELECT TABLE_NAME, TABLE_COMMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'exian'";
...
function drop_table(table) {
	if(confirm(table + " 테이블을 정말 삭제하시겠습니까? 삭제 후에는 복구가 불가능합니다")) {
		var data_string = "mode=drop&table=" + table;
		$.ajax({
			type : "post",
			data : data_string,
			url : "ajax/config.php",
			success : function(str) {
				if(str == "success") location.reload();
				else alert(str);
			}
		});
	}
...
function truncate_table(table) {
	if(confirm(table + " 테이블을 정말 초기화 하시겠습니까? 초기화 후에는 복구가 불가능합니다")) {
		var data_string = "mode=truncate&table=" + table;
		$.ajax({
			type : "post",
			data : data_string,
			url : "ajax/config.php",
